        ...
        if (platCustId == null)
        {
            ...
            catch (Exception ex)
            {
                SystemLog.error(this.getClass().getName(), ucEntService.getClass().getName() + ".getPlatCustId()",
                        TMErrorCode.TM_INVOKE_UC_NOT_QUERY_DATA.getCode() + ":调用用户中心接口异常，未查询到企业信息", ex);
                throw new HsException(TMErrorCode.TM_INVOKE_UC_NOT_QUERY_DATA, "调用用户中心接口异常，未查询到企业信息:"
