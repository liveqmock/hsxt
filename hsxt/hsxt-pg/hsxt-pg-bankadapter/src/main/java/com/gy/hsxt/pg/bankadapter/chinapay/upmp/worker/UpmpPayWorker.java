...
public class UpmpPayWorker extends AbstractBankWorker {
	...
	public String doGetPayTN(UpmpPayTnParam param) throws Exception {
		...
		reqMap.put(PayReqParamKey.CHARSET, BanksConstants.CHARSET_UTF8);// 字符编码
	...
	public BankPaymentOrderResultDTO parseResultFromBankCallback(
		...
		int pgTransType = CpBizValueMapSwapper.toPgTransType(cbParamMap
				.get(PayCallbackParamKey.TRANS_TYPE));
		...
		returnValue.setBankSeqNo(qn); // 查询流水号
		...
		// 01-交易处理中
			...
			returnValue.setOrderStatus(PaymentOrderStatus.PAY_PROCESSING);
		} else {
