...
public class RefundWorker extends AbstractB2cWorker {
	 ...
	 */
	public BankPaymentOrderResultDTO doRefund(ChinapayRefundParam params) throws Exception {
		// 进行退款的参数校验
		...
		if (CpRespCode.REQ_SUCCESS_0.equals(respCode)) {
			...
			BankPaymentOrderResultDTO returnValue = new BankPaymentOrderResultDTO(
					orderNo, PaymentOrderStatus.REFUND_FAILED);
			...
			if (verifySignatureOfRefund(returnMap)) {
				...
				// 3-退款成功
					...
					returnValue.setOrderStatus(PaymentOrderStatus.REFUND_SUCCESS);
				}
				// 8-退款失败
				else if (CpRefundStatus.FAILED_8.equals(refundStatus)) {
					returnValue.setOrderStatus(PaymentOrderStatus.REFUND_FAILED);
				} else {
	...
	public BankPaymentOrderResultDTO parseResultFromBankCallback (
		...
		// 将银行的交易类型枚举值适配成我们内部的交易类型枚举值
	    int pgTransType = CpBizValueMapSwapper.toPgTransType(BankTransType.REFUND);
		
		BankPaymentOrderResultDTO returnValue = new BankPaymentOrderResultDTO(ordId,
