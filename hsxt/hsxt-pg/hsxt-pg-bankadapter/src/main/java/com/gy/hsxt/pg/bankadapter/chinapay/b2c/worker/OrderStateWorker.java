...
public class OrderStateWorker extends AbstractB2cWorker {
	 ...
	 * @exception 异常
	 */
	public BankPaymentOrderResultDTO doQueryOrderState(String translateType, String orderNo,
	 ...
	 */
	private BankPaymentOrderResultDTO queryOrderState(String translateType,
	 ...
	 * @return
	 */
	private BankPaymentOrderResultDTO parseReturnedResult(String bankTransType,
		...
		if (CpRespCode.REQ_SUCCESS_0.equals(respCode)) {
			...
			PaymentOrderStatus orderStatus = this.getDefaultStatus(bankTransType);
			...
			// 1003退款提交成功
					...
					&& orderNo.equals(bankRespOrderNo)) {
				returnValue.setOrderStatus(PaymentOrderStatus.REFUND_PROCESSING);
			}
			// 1005退款撤销成功
			else if (BankTransStatus.REFUND_CANCEL_SUCCESS.equals(bankStatus)
					&& orderNo.equals(bankRespOrderNo)) {
				returnValue.setOrderStatus(PaymentOrderStatus.REFUND_CANCEL_SUCCESS);
			} else {
	 ...
	 * @return
	 */
	private PaymentOrderStatus getDefaultStatus(String translateType) {
		// 支付交易
