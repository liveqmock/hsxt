...
public class QuickPaySecondWorker extends AbstractUpopWorker {
	 ...
	 * @throws Exception
	 */
	public BankPaymentOrderResultDTO doSecondPayRequest(UpopPaySecondParam params)
			throws Exception {
		// 参数校验
	 ...
	 * @return
	 */
	private BankPaymentOrderResultDTO parseResponseResult(String returnHtmlContent) {
		// 将响应数据转换为map结构
		...
		if (UpopTransStatus.TRANS_SUCCESS.equals(transStatus)) {
			dto.setOrderStatus(PaymentOrderStatus.PAY_SUCCESS);
		} else if (UpopTransStatus.TRANS_UNKOWN.equals(transStatus)) {
			dto.setOrderStatus(PaymentOrderStatus.UNKNOWN);
			...
			dto.setOrderStatus(PaymentOrderStatus.PAY_FAILED);
