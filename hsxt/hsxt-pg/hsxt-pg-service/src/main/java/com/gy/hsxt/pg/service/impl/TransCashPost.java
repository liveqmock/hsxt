...
public class TransCashPost extends ParentTransCashPost {
	 ...
	 * @throws Exception
	 */
	private void actionSingleTransRequest(TPgBankTransOrder pinganOrder)
			throws Exception {
	...
	private void actionBatchTransRequest(String bankBatchNo,
			List<TPgBankTransOrder> pinganOrderList) throws Exception {
		// 是否是socket通信异常
		boolean isSocketError = false;
	 ...
	 * @param pinganOrder
	 */
	private void updateAndNotify4SingleTransState(TPgBankTransOrder pinganOrder) {
	...
	private boolean doNotifySingleTransferState(String bankOrderNo,
		...
		TPgBankTransOrder pinganOrder = convert2TPgPinganOrder(bankOrderNo,
	 ...
	 * @return
	 */
	private boolean notifyGPSingleTransCash(TPgBankTransOrder pinganOrder) {
	...
	private B2eBatchTransParam assembleBatchTransCashParam(String bankBatchNo,
		...
		// 组装批量请求参数对象
		for (TPgBankTransOrder pinganOrder : pinganOrderList) {
		...
		TPgBankTransOrder pinganOrder$ = pinganOrderList.get(0);
