...
import com.gy.hsxt.pg.service.impl.parent.ParentAccountingDSBatchService;
 ...
 * 
 *  File Name       : ChinapayAccountingDSBatchService.java
 * 
 ...
 * </PRE>
 ***************************************************************************/
@Service("chinapayAccountingDSBatchService")
public class ChinapayAccountingDSBatchService extends
		ParentAccountingDSBatchService {
	...
	@Autowired
	private IPaymentOrderStateSyncService orderStateSyncService;
	...
	private class ChinapayBalanceCallback implements IChinapayBalanceCallback {
		...
		// 当前支付网关关注的银联订单号特征表达式, 暂时废弃!! marked by:zhangysh
		...
		@Override
		public void notifyPageData(List<BankPaymentOrderResultDTO> pageDatas,
		...
		@Override
		public boolean isMyFocus(BankPaymentOrderResultDTO orderItem) {
			// 暂时废弃! marked by:zhangysh
			// StringUtils.avoidNull(orderItem.getOrderNo()).matches(orderIdRangeRegex);
		...
		private List<CHBalanceVo> convert2CHBalanceVo(
				List<BankPaymentOrderResultDTO> pageDatas) {

			List<CHBalanceVo> chBalanceVoList = new ArrayList<CHBalanceVo>(
					getRowsPerPage());

			if (null != pageDatas) {
				for (BankPaymentOrderResultDTO orderItem : pageDatas) {
		...
		private CHBalanceVo conver2CHGPBalanceVo(
				BankPaymentOrderResultDTO orderItem) {
