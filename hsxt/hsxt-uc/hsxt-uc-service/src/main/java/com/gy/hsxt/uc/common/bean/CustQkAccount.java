...
public class CustQkAccount implements Serializable{
    
	private static final long serialVersionUID = 3482653153883834397L;
	...
	/**
	 * @return the 消费者快捷支付签约账户编号（主键序列号）
	 */
	...
	/**
	 * AsQkBank 转换成 CustQkAccount
	 * 
	 * @param asQkBank
	 * @return
	 */
	public static CustQkAccount generateCustQkAccount(AsQkBank asQkBank) {
		CustQkAccount qkAcc = new CustQkAccount();
		BeanUtils.copyProperties(asQkBank, qkAcc);
		qkAcc.setPerCustId(asQkBank.getCustId());
		qkAcc.setPerResNo(asQkBank.getResNo());
	...
	public AsQkBank generateAsQkBank() {
		...
		asQkBank.setCustId(this.perCustId);
		asQkBank.setResNo(this.perResNo);
