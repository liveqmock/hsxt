 ...
 */
public class BatchSavePwdQuestion implements Runnable,IBatchRollback {
	/** 业务操作服务类*/
	UCAsCardHolderService uCAsCardHolderService;
	/** 业务操作参数*/
	List<PwdQuestion> dataList;
	 ...
	 */
	public BatchSavePwdQuestion(String key,UCBsCardHolderService parentService,UCAsCardHolderService uCAsCardHolderService,
			List<PwdQuestion> dataList,CountDownLatch latch) {
		this.key=key;
...
//		parentService.addRollbackServ((IBatchRollback)this);
		try {// do 批量入库
			uCAsCardHolderService.batchSavePwdQuestion(dataList);// 保存持卡人信息
		} catch (Exception e) {
			// 异常通知父线程，父线程会调用 rollback 处理脏数据
			parentService.addRollbackKey(key,e.getMessage());
			e.printStackTrace();
			SystemLog.error("BatchSavePwdQuestion", "run", "密保记录入库出错：", e);
