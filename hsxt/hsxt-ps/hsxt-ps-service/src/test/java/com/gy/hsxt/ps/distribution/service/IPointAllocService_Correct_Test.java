...
public class IPointAllocService_Correct_Test {
	...
	public void allocPoint() { 
		...
		try {
			...
			Pair<List<Alloc>,EntryAllot> list=pointAllocService.allocReturnPoint(correct, pointDetail);
			outList(list.getLeft());
			assertEquals(list.getLeft().size(), 4);
		} catch (HsException e) { 
	...
	public void allocPoint1(){
		...
		try {
			Pair<List<Alloc>,EntryAllot> list=pointAllocService.allocReturnPoint(correct,pointDetail);
			outList(list.getLeft());
			assertEquals(list.getLeft().size(), 5);
		} catch (HsException e) { 
			fail("ErrorCode:" + e.getErrorCode() + " ErrorMessage:" + e.getMessage() + "\n" + e.getStackTrace());
