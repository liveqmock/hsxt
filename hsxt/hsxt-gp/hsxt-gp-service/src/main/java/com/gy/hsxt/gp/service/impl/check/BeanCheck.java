...
public class BeanCheck {
	...
	/**
	 * 校验 PinganQuickPayBindingBean
	 * 
	 * @param cardBindingBean
	 * @throws HsException
	 */
	public static void isvalid(PinganQuickPayBindingBean cardBindingBean)
			throws HsException {
		
		if (null == cardBindingBean) {
			throw new HsException(PGErrorCode.INVALID_PARAM,
					"传递的PinganQuickPayBindingBean对象不能为空!");
		}
	...
	/**
	 * 校验PinganQuickPaySmsCodeBean
	 * 
	 * @param smsCodeBean
	 * @throws HsException
	 */
	public static void isvalid(PinganQuickPaySmsCodeBean smsCodeBean)
			throws HsException {
		...
		// 判断merId是否有效
		BasicPropertyCheck.checkMerId(smsCodeBean.getMerId());

		// 判断业务订单号orderNo是否有效
		BasicPropertyCheck.checkOrderNo(smsCodeBean.getOrderNo());

		// 判断交易日期orderDate是否有效
		BasicPropertyCheck.checkOrderDate(smsCodeBean.getOrderDate());

		// 判断交易金额transAmount是否有效
		BasicPropertyCheck.checkTransAmount(smsCodeBean.getTransAmount());

		// 判断交易描述transDesc是否有效
		BasicPropertyCheck.checkTransDesc(smsCodeBean.getTransDesc());

		// 判断币种currencyCode是否有效
		BasicPropertyCheck.checkCurrencyCode(smsCodeBean.getCurrencyCode());

		// 判断业务订单号privObligate是否有效
		BasicPropertyCheck.checkprivObligate(smsCodeBean.getPrivObligate());
	}
	 ...
	 * @throws HsException
	 */
	public static void isvalid(PinganQuickPayBean quickPayBean)
			throws HsException {
		isvalid(quickPayBean);
	}
	...
	public static void isvalid(NetPayBean netPayBean) throws HsException {
		...
		if (netPayBean instanceof PinganNetPayBean) {
			BasicPropertyCheck.checkGoodsName(netPayBean.getGoodsName());
		}
	}
