...
public abstract class ParentPaymentService {
	 ...
	 * @return
	 */
	protected TGpPaymentOrder compateOrderData(String channelProvider) {
		...
		tGpPaymentOrder.setChannelProvider(channelProvider);
	 ...
	 * @return
	 */
	protected List<PaymentOrderState> syncPaymentStateFromPG(String merId,
		...
		// 从PG支付网关查询支付单状态
		List<PGPaymentOrderState> paymentStatesFromPG = pgPaymentService
				.getPaymentOrderState(MerIdUtils.merId2Mertype(merId),
						bankOrderNoList);

		if (null != paymentStatesFromPG) {
			TGpPaymentOrder gpPaymentOrder;

			for (PGPaymentOrderState pgOrderState : paymentStatesFromPG) {
