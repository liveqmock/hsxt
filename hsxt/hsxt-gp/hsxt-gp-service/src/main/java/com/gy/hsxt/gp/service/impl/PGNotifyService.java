...
public class PGNotifyService extends AbstractAppContextAwareServcie implements
	...
	@Autowired
	private LCSBaseInfoCacheService baseInfoCacheService;
	...
	public boolean notifyQuickPayBindingNo(
		...
		List<TGpCustidBankcardMap> list = new ArrayList<TGpCustidBankcardMap>(1);
		...
		if (!StringUtils.isEmpty(bankOrderNo)) {
			// 查询数据库
			TGpCustidBankcardMap custidBankcard = custidBankcardMapper
					.selectByBankcardnoBankorderno(bankOrderNo, bankCardNo);

			if (null != custidBankcard) {
				list.add(custidBankcard);
			}
		} else {
			// 查询出卡对应关系列表
			list = custidBankcardMapper.selectByBankcardno(bankCardNo);
		}
		...
		// 通知UC
		if (null != list) {
			for (TGpCustidBankcardMap record : list) {
	...
	private boolean doNotifyQuickPayBindingNo(
			...
			TGpCustidBankcardMap custidBankcard) {
		try {
			int bankType;
			...
			String channelProvider = custidBankcard.getChannelProvider();

			// 平安银行快捷
			if (PGChannelProvider.PINGAN_PAY.equals(channelProvider)) {
				bankType = pgQuickPayBindingNo.getBankCardType();
				bankId = pgQuickPayBindingNo.getBankId();
				bankName = ""; // 从LCS查询
			}
			// 中国银联快捷
			else {
				bankType = custidBankcard.getBankCardType();
				bankId = custidBankcard.getBankId();
				bankName = baseInfoCacheService.queryBankNameFromLCS(bankId);
			}

			// 根据 从数据库查询的数据与网关通知的数据组装通知用户中心的数据
			QuickPayBindingNo bindingNo = new QuickPayBindingNo();
			bindingNo.setCustId(custidBankcard.getCustId());
			bindingNo.setBankId(bankId);
			bindingNo.setBankName(bankName);
			bindingNo.setBankCardType(bankType);
			bindingNo.setBankCardNo(pgQuickPayBindingNo.getBankCardNo());
			bindingNo.setBindingNo(pgQuickPayBindingNo.getBindingNo());
			bindingNo.setExpireDate(pgQuickPayBindingNo.getExpireDate());			
			bindingNo.setTransLimit(pgQuickPayBindingNo.getTransLimit());
			bindingNo.setSumLimit(pgQuickPayBindingNo.getSumLimit());

			// --------------通知用户中心--------------------
			return notifyBingNoService
					.notifyQuickPayBindingNo(bindingNo);
		} catch (Exception e) {
