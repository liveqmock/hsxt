	...
	$.charcount = {
		util: {
			...
			pluralize: function(count, singular/*, plural*/) {
				var plural = (arguments.length > 2) ? arguments[2] : singular; 
			...
			if(element.is(':charcountable')) {
				...
				if(settings.position != POSITION_NONE) {
					...
					if(settings.position == POSITION_BEFORE) {
						//countDisplay.attr("style","padding-right:10px");				 
						countDisplay.attr("style",settings.style || "padding-right:10px");
						...
						//countDisplay.attr("style","padding-left:10px");				 
						countDisplay.attr("style",settings.style || "padding-left:10px");
						countDisplay.insertAfter(element).addClass(settings.classPrefix + '-position-' + POSITION_AFTER);
					}
				}			
				
				element.bind('keyup keypress charcount change', function(evt) {
					...
					if(evt.type == 'keypress' || evt.type == 'keyup' || evt.type == 'charcount' || evt.type == 'change') {
						...
						if(settings.position != POSITION_NONE) {				
							...
							display.html(remaining >= 0 ? "还能输入<b style='color:red'>"+remaining+"</b>字" :"已超出规定字数<b style='color:red'>"+Math.abs(remaining) + '</b>个,请修改!');
						} 
					}
				}).trigger('charcount');
			}
