...
import com.gy.hsxt.gpf.fss.notify.runner.VerifyRunner;
 ...
 * @Description : TODO
 * @Author : chenm
 * @Date : 2015/10/23 16:09
 * @Version V3.0.0.0
 */
public class VerifyHandler extends AbstractNotifyHandler<FileDetail> {

    private Logger logger = LoggerFactory.getLogger(VerifyHandler.class);

    public VerifyHandler(ThreadPoolTaskExecutor jobExecutor) {
        super(jobExecutor);
    }
    ...
    protected boolean doDeal(List<FileDetail> details, FileDetailService fileDetailService) throws HsException {
        if (CollectionUtils.isNotEmpty(details)) {
            ...
            for (FileDetail detail : details) {
                startCount++;
                Future<FutureBean<FileDetail>> future = jobExecutor.submit(new VerifyRunner(detail));
                futures.add(future);
            }
            if (isFinished()) {
