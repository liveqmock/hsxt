		...
		SimpleTree : function(options) {
			...
			option._init = function() {
				...
				this.tree.find("ul ul[show='true']").show(); /*
																 ...
																 * true 的子级菜单
																 */
				this.tree.find("ul ul[show='true']").prev("li")
						.addClass("open"); /*
											 * 添加 show 属性为 true 的子级菜单父节点的 open
											 * 样式
											 */
			}/* option._init() End */
			...
			this.find("li").click(function() {
				var tihst=$(this);
				/*
				 * 当单击菜单项 <li> 1.触发用户自定义的单击事件，将该 <li> 标签中的第一个超链接做为参数传递过去
				 * 2.修改当前菜单项所属的子菜单的显示状态（如果等于 true 将其设置为 false，否则将其设置为 true）
				 * 3.重新初始化菜单
				 */
				var a = tihst.find("a")[0];
				if (typeof (a) != "undefined")
					option.click(a); /* 如果获取的超链接不是 undefined，则触发单击 */
				/*
				 * 如果当前节点下面包含子菜单，并且其 show 属性的值为 true，则修改其 show 属性为 false 否则修改其
				 * show 属性为 true
				 */
				if (tihst.next("ul").attr("show") == "true") {
					tihst.next("ul").attr("show", "false");
				} else {
					tihst.next("ul").attr("show", "true");
				}
				/* 初始化菜单 */
				option._init();
			});
