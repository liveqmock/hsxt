...
<body>
	<table width="100%" border="0" cellpadding="8" cellspacing="0">
		<tr style="text-align: left; background-image: url(../images/table_b.png); background-repeat: repeat-x; background-position: left center;">
			...
			<td width="93%">
			              语言代码：<select id="languageCodes_q"></select>&nbsp;
		                            错误信息代码：<input type="text" id="errorCode" >&nbsp;
				错误信息内容：<input type="text" id="errorMsg" >&nbsp;
	...
	<form id="listForm">
		<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#cccccc">
			<tr style="text-align: center; background-image: url(../images/table_b.png); background-repeat: repeat-x; background-position: left center;">
				...
				<th style="width: 25%">错误信息代码</th>
				<th style="width: 25%">错误信息内容</th>
		...
		<form id="editForm">
			<table width="100%" border="0" cellpadding="5" cellspacing="1">
				<tbody>
					...
					<tr>
						<td align="right">错误信息代码：</td>
						<td align="left">
							<input type="text" name="errorCode" reg="[^\s]">
						</td>
					</tr>
					<tr>
						<td align="right">错误信息内容：</td>
						<td align="left">
							<input type="text" name="errorMsg" reg="[^\s]">
						</td>
					</tr>
				</tbody>
			</table>
...
<script type="text/javascript">
	...
	var obj = {
					...
					function(data) {
						...
						if (list.length >= 1) {
							for ( var i = 0; i < list.length; i++) {
								...
								html += '<td title="errorCode">' + objValue.errorCode + '</td>';
								html += '<td title="errorMsg">' + objValue.errorMsg + '</td>';
								html += '<td><a href="#s" onclick="obj.edit(this);">修改</a> | <a href="#s" onclick="obj.del(\'' + objValue.languageCode + '\',\'' + objValue.errorCode + '\');">删除</a></td>';
								html += '</tr>';
							}
						}
						$('#tbd').html(html);
		...
		edit : function(ths) {
			...
			opens.openDiv('#editFormDiv', '修改错误信息', 500, 420, function(objs) {
				var isSubmit = true;
		...
		//删除
		del : function(languageCode,errorCode) {
	...
	$('#add').click(function() {
		//清空表单数据
		$('#editForm')[0].reset();
		opens.openDiv('#editFormDiv', '添加错误信息', 500, 460, function(objs) {
			var isSubmit = true;
