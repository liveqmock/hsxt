...
<body>
	<table width="100%" border="0" cellpadding="8" cellspacing="0">
		<tr style="text-align: left; background-image: url(../images/table_b.png); background-repeat: repeat-x; background-position: left center;">
			...
			<td width="93%">
			              分配对象：<input type="text" id="targetType" >&nbsp;
		                            账户类型：<input type="text" id="actType" >&nbsp;
				分配方式：<input type="text" id="assignMethod" >&nbsp;
	...
	<form id="listForm">
		<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#cccccc">
			<tr style="text-align: center; background-image: url(../images/table_b.png); background-repeat: repeat-x; background-position: left center;">
				<th style="width: 20%">分配对象</th>
				<th style="width: 20%">账户类型</th>
				<th style="width: 20%">分配比例</th>
				<th style="width: 20%">分配方式</th>
	...
	<div id="editFormDiv" style="width: 100%; display: none;">
		<form id="editForm">
			<table width="100%" border="0" cellpadding="5" cellspacing="1">
				<tbody>
					<tr>
						<td width="30%" align="right">分配对象：</td>
						<td width="70%" align="left">
							<input type="text" name="targetType" reg="[^\s]">
						</td>
					</tr>
					<tr>
						<td align="right">账户类型：</td>
						<td align="left">
							<input type="text" name="actType" reg="[^\s]">
						</td>
					</tr>
					<tr>
						<td align="right">分配比例：</td>
						<td align="left">
							<input type="text" name="assignRate" reg="[^\s]">
						</td>
					</tr>
					<tr>
						<td align="right">分配方式：</td>
						<td align="left">
							<input type="text" name="assignMethod" reg="[^\s]">
						</td>
					</tr>
				</tbody>
			</table>
...
<script type="text/javascript">
	...
	var obj = {
					...
					function(data) {
						...
						if (list.length >= 1) {
							for ( var i = 0; i < list.length; i++) {
								...
								html += '<td title="targetType">' + objValue.targetType + '</td>';
								html += '<td title="actType">' + objValue.actType + '</td>';
								html += '<td title="assignRate">' + objValue.assignRate + '</td>';
								html += '<td title="assignMethod">' + objValue.assignMethod + '</td>';
								html += '<td><a href="#s" onclick="obj.edit(this);">修改</a> | <a href="#s" onclick="obj.del(\'' + objValue.targetType + '\',\'' + objValue.actType + '\');">删除</a></td>';
								html += '</tr>';
							}
						}
						$('#tbd').html(html);
		...
		edit : function(ths) {
			...
			opens.openDiv('#editFormDiv', '修改积分分配比例', 500, 420, function(objs) {
				var isSubmit = true;
		...
		//删除
		del : function(targetType,actType) {
	...
	$('#add').click(function() {
		//清空表单数据
		$('#editForm')[0].reset();
		opens.openDiv('#editFormDiv', '添加计量单位', 500, 460, function(objs) {
			var isSubmit = true;
