 ...
 * @version V1.0
 */
public class ReportAccountEntGiveIntegrel implements Serializable{
	...
	public synchronized void write(StringBuilder writeFileData){
		String fileAdder = this.filePath+this.fileName;
		...
		if(sourceFile.exists() && sourceFile.length() >= maxFileSize){
			...
			this.fileName = FileNameBuilder.build(FssPurpose.BM_BMLM, newDate, fileNum, sysName);
			fileAdder = this.filePath+this.fileName;
		}
