 ...
 * 
 * @Package: com.gy.hsxt.rp.common.bean  
 * @ClassName: ReportsWelfareScoreExportFile 
 * @Description: 福利积分写文件信息
 *
 * @author: maocan 
 * @date: 2015-9-17 上午9:47:29
 * @version V1.0
 */
public class ReportsWelfareScoreExportFile implements Serializable{
	...
	public synchronized void write(StringBuilder writeFileData){
		String fileAdder = this.filePath+this.newMonth+"/"+this.newDate+"/"+this.fileName;
		...
		if(sourceFile.exists() && sourceFile.length() >= maxFileSize){
			...
			this.fileName = "WS_PS_DETAIL_"+this.fileNum+".TXT";
//			this.fileName = name+this.fileNum+".txt";
			fileAdder = this.filePath+this.newMonth+"/"+this.newDate+"/"+this.fileName;
		}
