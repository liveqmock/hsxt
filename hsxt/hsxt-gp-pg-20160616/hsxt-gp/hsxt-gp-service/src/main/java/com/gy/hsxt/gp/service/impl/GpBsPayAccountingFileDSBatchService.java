 ...
 * 
 *  File Name       : GpBsPayAccountingFileDSBatchService.java
 * 
 *  Creation Date   : 2016年2月19日
 * 
 *  Author          : zhangysh
 * 
 *  Purpose         : GP支付系统与BS对账文件生成调度接口(提供给DS系统调用)
 * 
 * 
 *  History         : none
 * 
 * </PRE>
 ***************************************************************************/
@Service("gpBsPayAccountingFileDSBatchService")
public class GpBsPayAccountingFileDSBatchService extends
	...
	protected String getTcChkFileNameKey() {
		return TcConfigConsts.KEY_SYS_TC_CHK_GP_BS_PAY;
	}
	...
	protected String getTcDetFileNameKey() {
		return TcConfigConsts.KEY_SYS_TC_DET_GP_BS_PAY;
	}
	 ...
	 * @return
	 */
	private List<GPBSPayBalanceVo> queryOrderDatasByOrderDate(String orderDate,
		...
		List<GPBSPayBalanceVo> paymentOrders = paymentOrderMapper
				.selectByOrderDate$GPBS(parasMap);

		// 加入结尾提示标识
		if (null != paymentOrders) {
			paymentOrders.add(new GPBSPayBalanceVo(true));
		}
	...
	private void actionCreateBalanceFile(String balanceDate,
		...
		// 每次从数据库中查出的记录
		List<GPBSPayBalanceVo> paymentOrders;
