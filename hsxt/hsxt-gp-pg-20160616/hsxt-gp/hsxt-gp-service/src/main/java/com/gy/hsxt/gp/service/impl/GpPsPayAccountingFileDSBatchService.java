 ...
 * 
 *  File Name       : GpPsPayAccountingFileDSBatchService.java
 * 
 *  Creation Date   : 2016年2月19日
 * 
 *  Author          : zhangysh
 * 
 *  Purpose         : GP支付系统与PS对账文件生成调度接口(提供给DS系统调用)
 * 
 * 
 *  History         : none
 * 
 * </PRE>
 ***************************************************************************/
@Service("gpPsPayAccountingFileDSBatchService")
public class GpPsPayAccountingFileDSBatchService extends
	...
	protected String getTcChkFileNameKey() {
		return TcConfigConsts.KEY_SYS_TC_CHK_GP_PS_PAY;
	}
	...
	protected String getTcDetFileNameKey() {
		return TcConfigConsts.KEY_SYS_TC_DET_GP_PS_PAY;
	}
	 ...
	 * @return
	 */
	private List<GPPSPayBalanceVo> queryOrderDatasByOrderDate(String orderDate,
		...
		List<GPPSPayBalanceVo> paymentOrders = paymentOrderMapper
				.selectByOrderDate$GPPS(parasMap);

		// 加入结尾提示标识
		if (null != paymentOrders) {
			paymentOrders.add(new GPPSPayBalanceVo(true));
		}
	...
	private int getCountByOrderDate(String orderDate, String srcSubsysId) {
		...
		return paymentOrderMapper.getcountByOrderDate$GPPS(dateMap);
	}
	...
	private void actionCreateBalanceFile(String balanceDate,
		...
		// 每次从数据库中查出的记录
		List<GPPSPayBalanceVo> paymentOrders;
