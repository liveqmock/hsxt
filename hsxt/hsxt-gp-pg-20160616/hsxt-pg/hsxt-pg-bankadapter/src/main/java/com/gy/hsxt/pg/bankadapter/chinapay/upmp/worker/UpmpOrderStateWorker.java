...
public class UpmpOrderStateWorker extends AbstractBankWorker {
	 ...
	 * @throws Exception
	 */
	private ChinapayOrderResultDTO getPayOrderState(String transType, String orderNo,
		...
		reqMap.put(OrderStateReqKey.CHARSET, BanksConstants.DEFAULT_CHARSET);// 字符编码
		...
		if (UpmpService.verifyResponse(respString, null, privateKey)) {
			...
			ChinapayOrderResultDTO returnValue = new ChinapayOrderResultDTO(orderNo,
			...
			if (CpUpmpRespCode.REQ_SUCCESS_00.equals(respCode)) {
				...
				if (CpUpmpTransStatus.TRANS_SUCCESS_00.equals(retTransStatus)
					...
					ChinapayOrderStatus orderStatus = (UpmpBankTransType.PAY
	...
	private ChinapayOrderStatus getDefaultStatus(String transType) {
