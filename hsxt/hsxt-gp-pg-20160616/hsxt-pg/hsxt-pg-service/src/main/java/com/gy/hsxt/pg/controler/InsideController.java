 ...
 *  Author          : zhangysh
 * 
 *  Purpose         : 处理来自地区平台内部的http请求
 *  
 *                    访问路径规则：http://127.0.0.1:8080/站点名称/paygateway/inner/upop/paysecond
 * 
 *  History         : none
 * 
 * </PRE>
...
public class InsideController {
	 ...
	 * 访问地址： http://127.0.0.1:8080/站点名称/paygateway/inner/upop/paysecond <br>
	 * 
	 * @param request
	 * @param response
	 * @return
	 * @throws Exception
	 */
	@RequestMapping("/" + PGUrlKeywords.UPOP_PAY_SECOND)
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			...
			logger.info("调用银联接口进行非首次支付出现异常, 详情：" + respData.getFailReason(),
					ex);
		}
	...
	private PGCallbackJumpRespData doHandleRequest(HttpServletRequest request)
		...
		if (!isBankOrderSubmited(chinapayOrder)
			...
			ChinapayOrderResultDTO resultDTO = chinapayUpopFacade
	...
	private PGCallbackJumpRespData assembleJumpRespData(
			TPgChinapayOrder chinapayOrder, ChinapayOrderResultDTO resultDTO,
			boolean isByRespFromBank) {
		...
		if (ChinapayOrderStatus.PAY_SUCCESS.valueEquals(transStatus)
				|| ChinapayOrderStatus.PAY_PROCESSING.valueEquals(transStatus)) {
			failReason = "";
	...
	private UpopPaySecondParam getUpopPaySecondParam(
			HttpServletRequest request, TPgChinapayOrder chinapayOrder) {
	 ...
	 * @return
	 */
	private boolean isBankOrderSubmited(TPgChinapayOrder chinapayOrder) {
	 ...
	 * @return
	 */
	private boolean isSmsCodeError(TPgChinapayOrder chinapayOrder) {
