...
@Singleton
public class ChinapayCardBindingService extends AbstractAppContextAwareServcie
		implements IChinapayCardBindingService {
	...
	protected void queryBindingNoFromCPByInterval() {
		logger.debug("---queryBindingNoFromCPByInterval enter---");
		...
		logger.debug("---queryBindingNoFromCPByInterval deep--");
			
		List<TPgBankcardBindingTmp> unknownBindingList;
	
		Date date = DateUtils.addMinutes(DateUtils.getCurrentDate(), -2);
		...
		while (true) {
			// 查询2分钟之前、签约状态为未知的记录
			unknownBindingList = bankcardBindingTemp$Mapper.selectByBindingStatus(
			...
			for (TPgBankcardBindingTmp tempRecord : unknownBindingList) {
				if (StringHelper.isEmpty(tempRecord.getBindingNo())) {
					// 从银联拉取签约号
					this.fetchAndNotifyBindingNoFromCP(tempRecord.getBankCardNo(),
							tempRecord.getBankId(), tempRecord.getBankCardType());
	
					// 考虑到银联的流量限制(35秒一次)
					ThreadHelper.sleepAmoment(35000);
				} else {
					tempRecord.setBindingStatus(PGQuickBindingStatus.IS_BINDING);
	 ...
	 * @param bankCardType
	 */
	private void insertTempBankCardBinding(String bankCardNo, String bankId,
			int bankCardType) {
	 ...
	 */
	private void updateBankCardBinding(
		...
		try {
			...
			String json = (null == pgQuickPayBindingNo) ? "" : JSON
					.toJSONString(pgQuickPayBindingNo);
	...
	private void doNotifyBindingNo(PGQuickPayBindingNo quickPayBindingNo,
		...
		try {
			...
			boolean success = getNotifyService(merType)
			...
			logger.info(StringHelper.joinString("通知GP同步签约号, 成功or失败?  结果："
					...
					JSON.toJSONString(quickPayBindingNo)));
	...
	private void fetchAndNotifyBindingNoFromCP(String bankCardNo,
		...
		if (logger.isInfoEnabled()) {
			logger.info("查询到的UPOP快捷支付签约号：" + JSON.toJSON(result));
		}
		...
		if (UpopCardBindingStat.SUCCESS.equals(result.getStat())) {
			...
			// 因为不知道在哪个支付系统, 所以一起通知：互生GP、互商GP
			int[] merTypes = new int[] { MerType.HSXT, MerType.HSEC };

			for (int merType : merTypes) {
				try {
					// 向UC用户中心进行签约号通知(通过互生系统的GP通知即可)
					boolean success = getNotifyService(merType)
							.notifyQuickPayBindingNo(quickPayBindingNo);
					...
					logger.info("通知GP同步签约号, 成功or失败?  结果：" + false
							+ ", merType=" + merType, e);
				}
			}	
		}
