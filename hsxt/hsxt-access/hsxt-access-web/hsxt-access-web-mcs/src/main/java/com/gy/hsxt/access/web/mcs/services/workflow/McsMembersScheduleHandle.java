 ...
 * @version V1.0
 */
public class McsMembersScheduleHandle implements Serializable {
     ...
     * 请求参数类
     */
    private McsMembersScheduleBean amsb;
    ...
    void convertMap() {
        ...
        //值班计划状态
        Object scheduleStatus= this.oMap.get("scheduleStatus");
        this.scheduleStatus = scheduleStatus == null ? null : Integer.parseInt(scheduleStatus.toString());
    }

    public McsMembersScheduleHandle() {
     ...
     * @param _oMap
     */
    public McsMembersScheduleHandle(McsMembersScheduleBean _amsb, Map<String, Object> _oMap) {
        this.amsb = _amsb;
        this.oMap = _oMap;
        ...
        if (this.solist != null)
        {
            ...
            for (ScheduleOpt so : this.solist)
            {
                if (so.getOptCustId().equals(o.getOptCustId()))
                {
                    ...
                    {
                        tempmsodt[Integer.parseInt(so.getPlanDate()) - 1] = WorkTypeStatus.getDescribe(so
                                .getWorkTypeTemp());
                    }
                }
            }
