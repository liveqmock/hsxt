...
import com.gy.hsxt.access.web.company.services.companyInfo.IPayBankService;
...
public class PayBankController extends BaseController {
    ...
    public HttpRespEnvelope findPayBanksByCustId(HttpServletRequest request) {
        try {
            ...
            RequestUtil.verifyParamsIsNotEmpty(
                new Object[] {entCustId, ASRespCode.AS_CUSTID_INVALID}
            );
    ...
    public HttpRespEnvelope addPayBank(HttpServletRequest request, @ModelAttribute AsQkBank bank) {
        try {
            ...
            //非空验证
            RequestUtil.verifyParamsIsNotEmpty(
                new Object[] {bank.getCustId(), ASRespCode.AS_ENT_CUSTID_INVALID},
                new Object[] {bank.getBankCode(), ASRespCode.AS_BANK_CODE_ERROR},
                new Object[] {bank.getBankName(), ASRespCode.AS_BANK_NAME_ERROR},
                new Object[] {bank.getBankCardNo(), ASRespCode.AS_BANKCARDNO_INVALID}
            );
            
            AsEntStatusInfo status = imemberEnterpriseService.searchEntStatusInfo(bank.getCustId());
            if (status != null)
            {
    ...
    public HttpRespEnvelope delPayBank(HttpServletRequest request) {
        try {
            ...
            //非空验证
            RequestUtil.verifyParamsIsNotEmpty(
                new Object[] {accId, ASRespCode.AS_BANKID_INVALID}
            );
            //删除快捷支付银行
