...
public class AnnualFeeController extends BaseController {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = { "/query_annualfee_info" }, method = {
	...
	public HttpRespEnvelope queryAnnualfeeInfo(HttpServletRequest request) {
		...
		if (httpRespEnvelope == null) {
			...
			try {
				Map<String, Object> result = iannualFeeManageService
						.findAnnualFeeShould(custId);
				...
				// 查询货币账户
				// AccountBalance hbBalance =
				// ibalanceService.findAccNormal(custId,
				// AccountType.ACC_TYPE_POINT30110.getCode());

				/** 企业账户互生币可用数量 **/
				result.put("availablePointNum",
						balance == null ? "0" : balance.getAccBalance());
				// 企业货币账户可用数量
				// result.put("availableHbNum", hbBalance == null ? "0" :
				// hbBalance.getAccBalance());
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = { "/pay_annualfee" }, method = { RequestMethod.GET,
			RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	public HttpRespEnvelope payAnnualfee(HttpServletRequest request,
			...
			try {
				...
				RequestUtil
						.verifyParamsIsNotEmpty(
								new Object[] {
										annualFeeOrder.getOrder()
												.getOrderOperator(),
										RespCode.MW_OPRATOR_OPTCUSTID.getCode(),
										RespCode.MW_OPRATOR_OPTCUSTID.getDesc() },
								new Object[] {
										annualFeeOrder.getOrder().getOrderNo(),
										RespCode.SW_ENT_ANNUALFEE_ORDERNO
												.getCode(),
										RespCode.SW_ENT_ANNUALFEE_ORDERNO
												.getDesc() },
								new Object[] {
										annualFeeOrder.getOrder()
												.getPayChannel(),
										RespCode.EW_TOOL_PAY_CHANNEL_INVALID
												.getCode(),
										RespCode.EW_TOOL_PAY_CHANNEL_INVALID
												.getDesc() }, new Object[] {
										annualFeeOrder.getOrder().getCustId(),
				...
				if (annualFeeOrder.getOrder().getPayChannel().intValue() == PayChannel.HS_COIN_PAY
						.getCode().intValue()) {
					RequestUtil.verifyParamsIsNotEmpty(new Object[] { tradePwd,
							RespCode.AS_TRADEPWD_INVALID.getCode(),
							RespCode.AS_TRADEPWD_INVALID.getDesc() },
							new Object[] { randomToken,
									RespCode.AS_SECURE_TOKEN_NULL.getCode(),
									RespCode.AS_SECURE_TOKEN_NULL.getDesc() });
				}
				// 快捷支付支付 验证交易密码 随机token 快捷支付预约号 短信验证码不能为空
				if (annualFeeOrder.getOrder().getPayChannel().intValue() == PayChannel.QUICK_PAY
						.getCode().intValue()) {
					RequestUtil.verifyParamsIsNotEmpty(new Object[] {
							annualFeeOrder.getSmsCode(),
							RespCode.EW_SMSCODE_INVALID.getCode(),
							RespCode.EW_SMSCODE_INVALID.getDesc() },
							new Object[] { annualFeeOrder.getBindingNo(),
									RespCode.EW_BINDINGNO_INVALID.getCode(),
									RespCode.EW_BINDINGNO_INVALID.getDesc() });
				}

				if (annualFeeOrder.getOrder().getPayChannel() == PayChannel.HS_COIN_PAY
						.getCode().intValue()) {
					// 呼叫中心的特殊处理方法
					tradePwd = RequestUtil
							.encodeBase64String(request, tradePwd);
					// 验证交易密码
					icommService.checkTradePwd(annualFeeOrder.getOrder()
							.getCustId(), tradePwd, UserTypeEnum.ENT.getType(),
							randomToken);
				}
				// 设置支付回调地址
				annualFeeOrder.setCallbackUrl(info.getWebPayJumpUrl());
				// 支付
				String url = iannualFeeManageService
						.addAnnualFee(annualFeeOrder);
				// 快捷支付支付 验证交易密码 随机token 快捷支付预约号 短信验证码不能为空
				if (annualFeeOrder.getOrder().getPayChannel().intValue() == PayChannel.QUICK_PAY
						.getCode().intValue()) {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = { "/create_annualfee_order" }, method = {
			RequestMethod.GET, RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	public HttpRespEnvelope createAnnualfeeOrder(HttpServletRequest request,
		...
		if (httpRespEnvelope == null) {
			try {
				...
				AnnualFeeOrder order = iannualFeeManageService
						.submitAnnualFeeOrder(annualFeeOrder);
