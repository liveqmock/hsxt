...
import com.gy.hsxt.access.web.scs.services.accountManagement.ICityTaxAccountService;
...
public class CityTaxAccountController extends BaseController {
    ...
    public HttpRespEnvelope queryTax(HttpServletRequest request,SCSBase scsBase) {
        HttpRespEnvelope hre = null;
        try
        {
            // Token验证
            super.checkSecureToken(request);
            //查询当前服务公司税率
            String taxRate=iCityTaxAccountService.queryTax(scsBase);
    ...
    public HttpRespEnvelope taxAdjustmentApply(HttpServletRequest request,ScsTaxrateChange atc,SCSBase scsBase) {
        HttpRespEnvelope hre = null;
        try
        {
            // Token验证
            ...
            //调整申请
            iCityTaxAccountService.taxAdjustmentApply(atc,scsBase);
    ...
    public HttpRespEnvelope taxApplyDetail(HttpServletRequest request,String applyId) {
        ...
        try
        {
            ...
            super.checkSecureToken(request);
            //验证有效数据
            RequestUtil.verifyParamsIsNotEmpty(new Object[]{applyId,RespCode.SW_APPLY_ID_NOT_NULL});
            //调整申请
     ...
     */
    @ResponseBody
    @RequestMapping(value = "get_proof_material_module", method = { RequestMethod.GET, RequestMethod.POST }, produces = "application/json;charset=UTF-8")
    public HttpRespEnvelope getProofMaterialModule(HttpServletRequest request) {
        HttpRespEnvelope hre = null;// 返回数据对象
        try
        {
            //检验token
            super.verifySecureToken(request);
            hre = new HttpRespEnvelope(scsConfigService.getTaxRateProofMaterialModule());
        }
        catch (Exception e)
        {
            hre = new HttpRespEnvelope(e);
    ...
    public HttpRespEnvelope getLastTaxApply(HttpServletRequest request,SCSBase scsBase) {
        HttpRespEnvelope hre = null;// 返回数据对象
        try
        {
            //检验token
            super.verifySecureToken(request);
            //获取最新审批结果
            TaxrateChange tch=iCityTaxAccountService.getLastTaxApply(scsBase);
