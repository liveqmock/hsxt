...
public class IntegralAccountController extends BaseController {
			...
			HttpServletRequest request)
	{
		...
		try
		{
			...
			// 保底积分数
			Double pliNumber = DoubleUtil.parseDouble(scsConfigService.getPersonLeastIntegration());
			...
			if (accBalance != null)
			{
				...
				// 保存账户余额
				map.put("usablePointNum", itnAccBalance < 0 ? 0 : itnAccBalance);
			}

			// 查询昨日积分数
			accEntrySum = balanceService.findEntIntegralByYesterday(entCustId,
			...
			// 今日积分数
			map.put("yesterdayPoint", sumAmount);// 保存昨日积分数
	...
	@ResponseBody
	@RequestMapping(value =
	{ "/init_integral_transfer_Hsb" }, method =
	...
	@ResponseBody
	@RequestMapping(value =
	{ "/integral_transfer_Hsb" }, method =
	{ RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	public HttpRespEnvelope integralTransferHsb(@ModelAttribute PvToHsb pvToHsb, String custEntName, String entCustId,
			String tradePwd, String randomToken, HttpServletRequest request)
	{
		...
		try
		{
			// Token验证
			super.checkSecureToken(request);
			// 呼叫中心调用的时候没有传递当前值，这句话不知道为什么加入的
			// pvToHsb.setCustName(custEntName);
			// 非空数据验证
			RequestUtil.verifyParamsIsNotEmpty(new Object[] { tradePwd, RespCode.PW_TRADEPWD_INVALID}, // 交易密码
					new Object[] { randomToken, RespCode.AS_SECURE_TOKEN_INVALID}, // 随机token
					new Object[] { pvToHsb.getHsResNo(), RespCode.AS_CUSTID_INVALID}, // 客户互生号
					new Object[] { entCustId, RespCode.AS_ENT_CUSTID_INVALID}, // 企业客户号
					new Object[] { pvToHsb.getCustName(), RespCode.PW_CUSTNAME_INVALID} // 客户名称
			);
			...
			// 转出的积分大于最小积分数量验证
			Double itcm = DoubleUtil.parseDouble(this.scsConfigService.getIntegrationConvertibleMin());
			if (DoubleUtil.parseDouble(pvToHsb.getAmount()) < itcm)
			{
				hre = new HttpRespEnvelope(RespCode.PW_INTEGRATIONCONVERTIBLEMIN_INVALID);
				return hre;
			}
			...
			pvToHsb.setCustType(CustType.SERVICE_CORP.getCode()); // 持卡人
	...
	@ResponseBody
	@RequestMapping(value =
	{ "/init_integral_investment" }, method =
	{ RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	...
	@ResponseBody
	@RequestMapping(value =
	{ "/integral_investment" }, method =
	{ RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	public HttpRespEnvelope integralInvestment(@ModelAttribute PointInvest pointInvest, String custEntName,
			String entCustId, String tradePwd, String randomToken, HttpServletRequest request)
	{
		...
		try
		{
			...
			// 呼叫中心调用的时候没有传递当前值，这句话不知道为什么加入的
			// pointInvest.setCustName(custEntName);
			// 非空数据验证
			RequestUtil.verifyParamsIsNotEmpty(new Object[]
			...
			{ entCustId, RespCode.AS_ENT_CUSTID_INVALID.getCode(), RespCode.AS_ENT_CUSTID_INVALID.getDesc() }, // 企业客户号
					new Object[]
			{ pointInvest.getHsResNo(), RespCode.AS_CUSTID_INVALID.getCode(), RespCode.AS_CUSTID_INVALID.getDesc() }, // 客户互生号
					new Object[]
			{ pointInvest.getCustName(), RespCode.SW_ENT_FILING_ENTCUSTNAME_INVALID.getCode(),
					RespCode.SW_ENT_FILING_ENTCUSTNAME_INVALID.getDesc() } // 客户名称
			);
			...
			// 积分账户的可用积分数可转出，转出积分数为不小于的整数！
			Double itim = DoubleUtil.parseDouble(this.scsConfigService.getIntegrationInvIntMult());
			if (DoubleUtil.parseDouble(pointInvest.getInvestAmount()) < itim)
			{
			...
			pointInvest.setCustType(CustType.SERVICE_CORP.getCode()); // 持卡人
	...
	public HttpRespEnvelope getAccOptDetailed(HttpServletRequest request, HttpServletResponse response)
	{
		...
		try
		{
			...
			Map<String, Object> map = balanceService.queryAccOptDetailed(transNo, transType);
			// 获取本平台的信息
	...
	@ResponseBody
	@RequestMapping(value =
	{ "/get_point_allot_detailed" }, method =
	{ RequestMethod.GET, RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	public HttpRespEnvelope getPointAllotDetailed(String entResNo, String batchNo,HttpServletRequest request)
	{
		HttpRespEnvelope hre = null;
		...
		try
		{
			// Token验证
			super.verifySecureToken(request);
			AllocDetailSum pdr = balanceService.queryPointAllotDetailed(batchNo,
					entResNo);
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "/get_point_allot_detailed_list")
