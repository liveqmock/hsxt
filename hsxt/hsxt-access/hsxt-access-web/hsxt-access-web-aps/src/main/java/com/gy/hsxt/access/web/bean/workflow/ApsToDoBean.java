 ...
 * 
 * @author: wangjg
 * @date: 2015-12-19 下午12:09:05
 * @version V1.0
 */
public class ApsToDoBean extends AbstractAPSBase implements Serializable {
    private static final long serialVersionUID = -2066669371961461627L;
    ...
    public void checkData() {
        ...
        RequestUtil.verifyParamsIsNotEmpty(
                new Object[] { this.getAssignedMode(), ASRespCode.APS_ASSIGNEDMODE_NOT_NULL},
                new Object[] { this.taskIdStr, ASRespCode.APS_BIZNO_NOT_NULL}
                );

        // 验证工单list空、长度
        if (null == this.taskIds || this.taskIds.size() == 0)
        {
            throw new HsException(ASRespCode.APS_BIZNO_NOT_NULL);
        }

        // 手动指派时，验证值班人员数据
        if (SendOrderPattern.MANUAL_SEND.getCode().intValue() == this.getAssignedMode())
        {
            RequestUtil.verifyParamsIsNotEmpty(new Object[] { this.optCustIdStr, ASRespCode.APS_OPTCUST_NOT_NULL });

            // 验证值班员list空、长度
            if (null == this.optCustIds || this.optCustIds.size() == 0)
            {
                throw new HsException(ASRespCode.APS_OPTCUST_NOT_NULL);
            }
        }
