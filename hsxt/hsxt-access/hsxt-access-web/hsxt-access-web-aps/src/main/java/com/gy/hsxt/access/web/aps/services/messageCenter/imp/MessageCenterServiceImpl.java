...
*/
package com.gy.hsxt.access.web.aps.services.messageCenter.imp;
...
import com.gy.hsxt.access.web.aps.services.messageCenter.IMessageCenterService;
...
public class MessageCenterServiceImpl extends BaseServiceImpl implements IMessageCenterService {
    ...
    public PageData<Message> findScrollResult(Map filterMap, Map sortMap, Page page) throws HsException {
    	...
    	SystemLog.info("地区平台","分页查询","page="+JSON.toJSONString(page)+" param="+JSON.toJSONString(param));
    	PageData<Message> pd = bsMsgService.queryMessageListPage(page, param);
    	return pd;
    }
    ...
    public void createMsgSend(Message msg,boolean isSend,String token) throws HsException {
    	...
    	try {
			String path = this.uploadFile(msg, isSend, token);
			msg.setFileId(path);
		} catch (FsException e) {
			throw new HsException(ASRespCode.AS_OPT_FAILED);
		}
    	SystemLog.info("地区平台", "发送消息", JSON.toJSONString(msg));
    ...
    public void editMsg(Message msg,boolean isSend,String token) throws HsException {
    	...
    	SystemLog.info("地区平台", "修改消息", JSON.toJSONString(msg));
    ...
    public Message query(String msgId) throws HsException {
    	...
    	SystemLog.info("地区平台", "单笔查询", "msgId="+msgId+"  result ="+JSON.toJSONString(msg));
    	return msg;
    }

	@Override
	public void deleteMessages(List<String> msgIds) throws HsException {
		SystemLog.info("地区平台", "批量删除消息", JSON.toJSONString(msgIds));
