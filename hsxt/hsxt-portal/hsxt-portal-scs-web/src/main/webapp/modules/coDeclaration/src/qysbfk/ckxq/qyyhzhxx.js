define(['text!coDeclarationTpl/qysbfk/ckxq/qyyhzhxx.html',
			'text!coDeclarationTpl/qysbfk/ckxq/qyyhzhxx_dialog.html',
	...
	return {	 	
			...
			$('#ckxq_xg').css('display','');
		 	...
		 	$('#ckxq_xg').click(function(){
		 		if($(this).text() == '修　改' ){
		 			...
		 			$('.tabList li >a[class="active"]').parent().click();
		 		}		 		
		 	});
		...
		initData : function(){
			var self = this;
			dataModoule.findBankByApplyId({"applyId": $("#editApplyId").val()}, function(res){
				cacheData = null;
				cacheData = res.data;
		...
		initShowForm : function(data, isCallBack){
			comm.delCache('coDeclaration', 'qysbfk-qyyhzhxx-info');
			comm.setCache('coDeclaration', 'qysbfk-qyyhzhxx-info', data);
			$('#ckxq_view').html('').append(_.template(qyyhzhxxTpl, data));
			var prov = null;
			var city = null;
			var comboxVal = null;
			if(data.bank){
		...
		initChangeData : function(){
			...
			//获取银行名称
			cacheUtil.findCacheBankAll(function(bankRes){
				var odata = comm.getCache('coDeclaration', 'qysbfk-qyyhzhxx-info');
