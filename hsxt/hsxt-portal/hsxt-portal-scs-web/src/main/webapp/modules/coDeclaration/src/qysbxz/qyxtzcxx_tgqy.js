define(['text!coDeclarationTpl/qysbxz/qyxtzcxx_tgqy.html',
        'coDeclarationDat/qysbxz/qyxtzcxx_tgqy',
        'coDeclarationSrc/qysbxz/point_choose',
        'coDeclarationLan'], function(qyxtzcxx_tgqyTpl, dataModoule, pointChoose){
	var scs_qysbxz_qyxtzcxx_tgqy = {
		showPage: function () {
			$('#contentWidth_2').html(_.template(qyxtzcxx_tgqyTpl));
			scs_qysbxz_qyxtzcxx_tgqy.initForm();
			scs_qysbxz_qyxtzcxx_tgqy.initData();
		},
		/**
		 * 获取申请编号
		 */
		...
		initForm: function () {
			...
			//初始化启用资源类型
			comm.initSelect('#toBuyResRange', comm.lang("coDeclaration").toBuyResRangeEnum, 205).change(function (e) {
				scs_qysbxz_qyxtzcxx_tgqy.findResNoListAndQuota(true, null);
			});
			...
			//$('#codPersonType').selectList({width:205});
			...
			$('#refincrement').click(function () {
				...
				if (zzdqyglh.substring(0, 5) == serviceNo.substring(0, 5)) {
					scs_qysbxz_qyxtzcxx_tgqy.findIncrement(zzdqyglh, false, true);
				} else {
			...
			$('#qyxtzcxx_save').click(function () {
				scs_qysbxz_qyxtzcxx_tgqy.saveRegInfoStepOne(false);
			});

			//判断页面是否有更新（编辑时候用）
			//$("#qyxtzcxx_tgqy_form :input").change(function() {
			//comm.setCache("coDeclaration", "updateFlag", true);
			//});

			//下一步 
			$('#qyxtzcxx_next').click(function () {
				scs_qysbxz_qyxtzcxx_tgqy.saveRegInfoStepOne(true);
			});
			...
			scs_qysbxz_qyxtzcxx_tgqy.initPCList(comm.getCustPlace()[1], comm.getCustPlace()[2]);
		},
		/**
		 * 初始化地区下拉
		 * @param prov 省份代码
		 ...
		 */
		initData: function () {
			var applyId = scs_qysbxz_qyxtzcxx_tgqy.getApplyId();
			if (applyId == null) {
				var entDeclare = comm.getCache("coDeclaration", "entDeclare");
				$("#sxxp1").attr("checked", "checked");
				if (entDeclare.defaultIncrement) {
					scs_qysbxz_qyxtzcxx_tgqy.fillTable(entDeclare.defaultIncrement);
				} else {
					scs_qysbxz_qyxtzcxx_tgqy.findIncrement("", true, false);
				}
				scs_qysbxz_qyxtzcxx_tgqy.findResNoListAndQuota(false, null);
			} else {
				dataModoule.findDeclareByApplyId({applyId: applyId}, function (res) {
					...
					if (res.data.toBuyResRange) {
						...
						scs_qysbxz_qyxtzcxx_tgqy.findResNoListAndQuota(false, res.data.toEntResNo);
					}
...
//					}
					...
					scs_qysbxz_qyxtzcxx_tgqy.initPCList(res.data.provinceNo, res.data.cityNo);
					if (res.data.toPnodeResNo) {
						...
						scs_qysbxz_qyxtzcxx_tgqy.findIncrement(res.data.toPnodeResNo, false, false, chooseId);
					} else {
						scs_qysbxz_qyxtzcxx_tgqy.findIncrement("", true, false);
					}
				});
				...
				dataModoule.findIncrement({resNo: resNo}, function (res) {
					if (res.data) {
						if (!res.data.incrementMap || $.isEmptyObject(res.data.incrementMap)) {
							...
							scs_qysbxz_qyxtzcxx_tgqy.fillTable(res.data.incrementMap);
							if (isAlert) {
								comm.alert({
									content: comm.lang("coDeclaration").refSuccess, callOk: function () {
									}
...
//					},
		...
		saveRegInfoStepOne: function (autoNext) {
			...
			var scs_qysbxz_qyxtzcxx_tgqy = this;
			var array = this.getPnode();//获取积分增值点设置
			if (array && array.length > 0) {
				scs_qysbxz_qyxtzcxx_tgqy.saveRegInfo(autoNext, array);
			}
		},
		/**
		 * 保存注册信息
...
//			params.toEntEnName = $("#toEntEnName").val();//被申报企业英文名称
			...
			dataModoule.checkValidEntResNo(params, function (result) {
				if (result.data && result.data == true) {//可用
					dataModoule.saveDeclare(params, function (res) {
						scs_qysbxz_qyxtzcxx_tgqy.saveStep(1, res.data);
					...
					scs_qysbxz_qyxtzcxx_tgqy.iConfirm(comm.lang('common')[12544], function () {
						dataModoule.saveDeclare(params, function (res) {
							scs_qysbxz_qyxtzcxx_tgqy.saveStep(1, res.data);
							comm.delCache("coDeclaration", "updateFlag");
							if (autoNext) {
	...
	return scs_qysbxz_qyxtzcxx_tgqy;
}); 
