define(['text!coDeclarationTpl/qysbcx/tgqysbtj.html',
        'coDeclarationSrc/qysbxz/tab',
        'coDeclarationDat/qysbcx/tgqysbtj',
        'coDeclarationLan'],function(tgqysbtjTpl, qysbxzTab, dataModoule){
	return {	 	
		showPage : function(){
			var self = this;
			$('#cx_content_list').html(_.template(tgqysbtjTpl)) ;
			...
			if(status == 0){
				return $('<a data-sn="'+record['entResNo']+'">编辑</a>').click(function(e) {
					dataModoule.findDeclareStep({applyId:record['applyId']}, function(res){
						qysbxzTab.showPage(record['applyId'], res.data, 3);
					});
				});
			}
			return null;
