define(['text!accountManageTpl/hsbzh/dhhsb.html',
		'text!accountManageTpl/hsbzh/payment.html',
		'accountManageDat/hsbzh/hsbzh',
		], function(tpl, paymentTpl,accountManageAjax){
	return dhhsb = {
		restrictData : null, 	//业务限制数据
		showPage : function(){
			$('#busibox').html(tpl).append(paymentTpl);
			...
			accountManageAjax.init_transfer_hsb(jsonParam,function (response) {
				if (response) {
					...
					dhhsb.restrictData = response.data.restrict;						//获取业务限制数据
				}
			})
			...
			$('#acpointQrBtn').click(function(){
				...
				jsonParam.entCustType = comm.getCookie('entCustType');
			...
			return comm.valid({
				...
				messages : {
					acpoint_appBuyCpointNum : {
						...
						huobi : comm.lang("accountManage").hsbBuy.digits,
