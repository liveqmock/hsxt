define(['text!workflowTpl/gdcx/tab.html',
	...
	return {
		showPage : function(){
			...
			var match = comm.findPermissionJsonByParentId("050501000000");
			
			//遍历节余款管理的子菜单设置默认选中
			for(var i = 0 ; i< match.length ; i++)
			 {
				 //办结工单一览
				 if(match[i].permId =='050501010000')
				 {
					 $('#bjgdyl').show();
					 $('#bjgdyl').click();
					 //已经设置默认值
				 ...
				 //挂起工单一览
				 else if(match[i].permId =='050501020000')
				 {
					 $('#gqgdyl').show();
				 ...
				 //拒绝受理工单一览
				 else if(match[i].permId =='050501030000')
				 {
					 $('#jjslgdyl').show();
					 if(isModulesDefault == false )
					 {
				 ...
				 //办理中工单一览
				 else if(match[i].permId =='050501040000')
				 {
					 $('#blzgdyl').show();
					 if(isModulesDefault == false )
					 {
				 ...
				 //未分配工单一览
				 else if(match[i].permId =='050501050000')
				 {
					 $('#wfpgdyl').show();
					 if(isModulesDefault == false )
					 {
