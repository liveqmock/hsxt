	...
	var rcapFun = {	
		...
		buttonBind : function(){
					...
					if ($(this).text()=='+'){
						...
						workflow.isChief(function(rsp){
							...
							if(!rsp.data && !adminBool){
								comm.alert({imgClass: 'tips_i' ,content: comm.lang("workflow")[36548]});		
								return false;
							}
								...
								buttons:{ 
									"创建":function(){ 
										...
										scheduleComm.addGroup(function(){
											...
											$("#050500000000_subNav_050502000000").click();//重新加载工作组
										});
									},
									"取消":function(){
										 $(this).dialog( "destroy" );
				...
				$('#zr_down').unbind("click").bind("click",function(e){
					...
					var param={"groupName":$group.text(),"groupId":$group.attr("data-id"),
							"year":self.currYear,"month":self.currMonth};
					
					//增加token验证参数
					param.token = comm.getRequestParams()["token"];
					param.custId = comm.getRequestParams()["custId"];
					param.channelType = comm.getRequestParams()["channelType"];
					
					window.open(workflow.exportSchedule(param),'_blank') 
				});
