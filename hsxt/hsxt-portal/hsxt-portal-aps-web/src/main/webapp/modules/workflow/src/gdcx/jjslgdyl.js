define(['text!workflowTpl/gdcx/jjslgdyl.html',
		...
		"workflowLan"
		], function(jjslgdylTpl, gqgdyl_zrdb_dialogTpl,zzgd_dialogTpl, gqgd_dialogTpl,workflow,gdcxcomm){
	var workFlowFun = {
		WFBsgrid:null,			//分页函数
		custIdIsChief:null, 	//当前操作员值班主任状态 true:是，false:否
		showPage : function(){
			$('#busibox').html(_.template(jjslgdylTpl));	
			...
			workflow.isChief(function(rsp){
				...
				//查询
				$('#wslgdqry_btn').unbind('click').bind('click',function(){
				...
				if(workFlowFun.custIdIsChief){
					...
					$('#plzrdb_btn').click(function(){
						var dataObj =workFlowFun.WFBsgrid.getCheckedRowsRecords();	//获取选中行
		...
		chuiBan : function(obj){/*
			comm.confirm({
				...
				callOk : function(){
					//工单催办
					var param={taskId:obj.taskId,reason:$("#txtReason").val()};
		...
		*/},
		...
		guaQi : function(obj){
			comm.confirm({
				...
				callOk : function(){
					obj.bizTypeName=gdcxcomm.getBizAuthName(obj.bizType);
					$('#gd_dialog').html(_.template(gqgd_dialogTpl,obj));
					
					//文本框转换为密码框(避免浏览器默认填充密码和用户名)
					$("#txtCheckPwd").focus(function(){
		...
		searchOperation : function(){
			...
			function(record, rowIndex, colIndex, options){
				...
				if(workFlowFun.custIdIsChief && colIndex==7){
			...
			function(record, rowIndex, colIndex, options){
				if(colIndex==7 && workFlowFun.custIdIsChief){
