define(['text!coDeclarationTpl/kqxtqksh/qysczl.html',
        'text!coDeclarationTpl/kqxtqksh/confirm_dialog.html',
        'coDeclarationDat/kqxtqksh/qysczl',
		'coDeclarationLan'], function(qysczlTpl, confirm_dialogTpl, dataModoule){
	var aps_kqxtqksh_qysczl = {
		creType:null,
		showPage: function () {
			aps_kqxtqksh_qysczl.initData();
		},
		/**
		 * 初始化界面
		 */
		initForm: function () {
			...
			$('#qysczl_xg_btn').click(function () {
				aps_kqxtqksh_qysczl.initChangeData();
			});
			//点击修改页面取消按钮
		...
		initData: function () {
			dataModoule.findAptitudeByApplyId({"applyId": $("#applyId").val()}, function (res) {
				...
				aps_kqxtqksh_qysczl.initShowForm(res.data, false);
			});
		},
		/**
		 * 初始化显示页面
		...
		initShowForm: function (data) {
			...
			aps_kqxtqksh_qysczl.creType=data.creType;
			...
			if(aps_kqxtqksh_qysczl.creType&&aps_kqxtqksh_qysczl.creType==2) {
				$('#case-fileId-1').hide();
				$('#preview-2-div').hide();
		...
		validateData: function () {
			...
			if (aps_kqxtqksh_qysczl.creType==2||$("#real-fileId-2").val() != "") {
		...
		initChangeData: function () {
			var aps_kqxtqksh_qysczl = this;
			if (!aps_kqxtqksh_qysczl.validateData().form()) {
				return;
			}
			...
			aps_kqxtqksh_qysczl.getChangePic("#lrCredentialFrontFileId", "#real-fileId-1", "法人代表证件正面", 1);
			if(aps_kqxtqksh_qysczl.creType==null || aps_kqxtqksh_qysczl.creType != 2) {
				aps_kqxtqksh_qysczl.getChangePic("#lrCredentialBackFileId", "#real-fileId-2", "法人代表证件反面", 2);
			}
			aps_kqxtqksh_qysczl.getChangePic("#busLicenceFileId", "#real-fileId-3", "营业执照扫描件", 3);
			aps_kqxtqksh_qysczl.getChangePic("#organizationFileId", "#real-fileId-6", "组织机构代码证扫描件", 6);
			aps_kqxtqksh_qysczl.getChangePic("#taxplayerFileId", "#real-fileId-7", "税务登记证扫描件", 7);
			aps_kqxtqksh_qysczl.getChangePic("#venBefProtocolId", "#real-fileId-8", "创业帮扶协议", 8);
			...
			//提交
			$('#qysczl_dialog > p').empty().html(_.template(confirm_dialogTpl));
			...
			aps_kqxtqksh_qysczl.initVerificationMode();
			$('#qysczl_dialog').dialog({
				width: 800,
				buttons: {
					'确定': function () {
						if (!aps_kqxtqksh_qysczl.validateViewMarkData().form()) {
							return;
						}
						//验证双签
						comm.verifyDoublePwd($("#doubleUserName").val(), $("#doublePassword").val(), 1, function (verifyRes) {
							...
							comm.uploadFile(null, ids, function (data) {
								...
								aps_kqxtqksh_qysczl.saveData(ndata, chg, verifyRes);
							}, function (err) {
								aps_kqxtqksh_qysczl.initTmpPicPreview();
		...
		saveData: function (ndata, chg, verifyRes) {
			...
			params.creType = aps_kqxtqksh_qysczl.creType;
			...
			dataModoule.saveDeclareAptitude(params, function (res) {
				...
				comm.alert({
					content: comm.lang("coDeclaration")[22000], callOk: function () {
						...
						aps_kqxtqksh_qysczl.initShowForm(ndata, true);
					}
				});
			});
		},
