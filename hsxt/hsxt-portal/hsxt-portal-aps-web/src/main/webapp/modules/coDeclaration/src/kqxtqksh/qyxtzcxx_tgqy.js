define(['text!coDeclarationTpl/kqxtqksh/qyxtzcxx_tgqy.html',
    ...
    var kqxtqksh_tgqy = {
        ...
        initForm: function (data) {
            ...
            comm.initSelect('#toBuyResRange', comm.lang("coDeclaration").toBuyResRangeEnum, 185).change(function (e) {
                ...
                kqxtqksh_tgqy.findResNoListAndQuota(spreadEntResNo, buyResRange, null);
            });
            ...
            $('#refincrement').click(function () {
                ...
                if (spreadEntResNo.substring(0, 2) == zzdqyglh.substring(0, 2)) {
                    kqxtqksh_tgqy.findIncrement(spreadEntResNo, zzdqyglh, true);
                } else {
            ...
            $('#qyxtzcxx_save').click(function () {
                kqxtqksh_tgqy.saveRegInfoStepOne();
            });
            ...
            kqxtqksh_tgqy.findResNoListAndQuota(data.spreadEntResNo, data.toBuyResRange, data.toEntResNo);
        },
        /**
         * 初始化数据
         */
        initData: function (data) {
            ...
            if (data.toPnodeResNo) {
                kqxtqksh_tgqy.toPnodeCustId = comm.removeNull(data.toPnodeCustId);//修改前被申报增值节点父节点客户号
                kqxtqksh_tgqy.toPnodeResNo = comm.removeNull(data.toPnodeResNo);//修改前被申报增值节点父节点资源
                kqxtqksh_tgqy.toInodeResNo = comm.removeNull(data.toInodeResNo);//修改前被申报选择增值节点
                kqxtqksh_tgqy.toInodeLorR = comm.removeNull(data.toInodeLorR);//修改前被申报选择增值节点对应区域
                ...
                kqxtqksh_tgqy.findIncrement(data.spreadEntResNo, data.toPnodeResNo, false, chooseId);
            }
        },
        /**
         * 查询企业配额数和可用互生号列表
        ...
        findResNoListAndQuota: function (spreadEntResNo, buyResRange, resNo) {
            ...
            dataModoule.findResNoListAndQuota(params, function (res) {
                if (resNo) {
                    kqxtqksh_tgqy.setHtmlData(resNo, res.data.availQuota, res.data.serCustName, res.data.serCustId);
                } else {
                    var chkType = $('input[name="chkType"]:checked').val();
                    if (chkType == '0') {
                        kqxtqksh_tgqy.setHtmlData(res.data.defaultEntResNo, res.data.availQuota, res.data.serCustName, res.data.serCustId);
                    } else {
                        kqxtqksh_tgqy.setHtmlData("", res.data.availQuota, res.data.serCustName, res.data.serCustId);
                    }
                }
            });
        },
         ...
         * @param mangeCompanyName 所属服务公司企业客户号
         */
            ...
            dataModoule.findIncrement({spreadEntResNo:spreadEntResNo,subResNo: subResNo}, function (res) {
                if (res.data) {
                    if (!res.data.incrementMap || $.isEmptyObject(res.data.incrementMap) || res.data.subRelation == false) {
                        ...
                        kqxtqksh_tgqy.fillTable(res.data.incrementMap);
                        if (chooseId) {
        ...
        saveRegInfoStepOne: function () {
            ...
            if (array && array.length > 0) {
                kqxtqksh_tgqy.saveRegInfo(array);
            } else {
        ...
        saveRegInfo: function (array) {
            ...
            //增值系统位置企业互生号
            var oldPnodeResNo = comm.removeNull(kqxtqksh_tgqy.toPnodeResNo);//修改前增值系统位置企业互生号
            ...
            //增值分配点
            var oldInodeResNo = comm.removeNull(kqxtqksh_tgqy.toInodeResNo);//修改前增值分配点
            var newInodeResNo = comm.removeNull(array[2]);//修改后增值分配点
            if (oldInodeResNo != newInodeResNo) {
                var oldInodeResNoText = kqxtqksh_tgqy.getIncrementScore(oldInodeResNo) + "增值分配点";//修改前增值分配点
                var newInodeResNoText = kqxtqksh_tgqy.getIncrementScore(newInodeResNo) + "增值分配点";//修改后增值分配点
                trs += "<tr><td class=\"view_item\">增值分配点</td><td class=\"view_text\">" + oldInodeResNoText + "</td><td class=\"view_text\">" + newInodeResNoText + "</td></tr>";
                chg.toInodeResNo = {"old": kqxtqksh_tgqy.toInodeResNo, "new": newInodeResNo};
                ndata.toInodeResNo = newInodeResNo;
            }

            //增值区
            var oldInodeLorR = comm.removeNull(kqxtqksh_tgqy.toInodeLorR);//修改前增值分配点
            ...
            //被申报增值节点父节点客户号
            var oldPnodeCustId = kqxtqksh_tgqy.toPnodeCustId;//修改前被申报增值节点父节点客户号
            ...
            kqxtqksh_tgqy.initVerificationMode();
            $('#qyxtzcxx_tj_dialog').dialog({
                width: 800,
                buttons: {
                    '确定': function () {
                        if (!kqxtqksh_tgqy.validateViewMarkData().form()) {
                            return;
                        }
                        //验证双签
                        comm.verifyDoublePwd($("#doubleUserName").val(), $("#doublePassword").val(), 1, function (verifyRes) {
                            ...
                            dataModoule.saveDeclare(ndata, function (res) {
                                ...
                                kqxtqksh_tgqy.toPnodeCustId = array[0];//修改后被申报增值节点父节点客户号
                                kqxtqksh_tgqy.toPnodeResNo = array[1];//修改后被申报增值节点父节点资源
                                kqxtqksh_tgqy.toInodeResNo = array[2];//修改后被申报选择增值节点
                                kqxtqksh_tgqy.toInodeLorR = array[3];//修改后被申报选择增值节点对应区域
                                comm.alert({
