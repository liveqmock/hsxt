define(['text!coDeclarationTpl/kqxtyw/qyxtzcxx_cyqy.html',
    ...
    var kqxtyw_cyqy = {
        ...
        showPage: function (data) {
            odata = data;
            pdata = data;
            kqxtyw_cyqy.applyId = data.applyId;
            kqxtyw_cyqy.initData(data);
        },
        /**
         * 获取申请编号
         */
        ...
        initForm: function (data) {
            ...
            comm.initSelect('#toBuyResRange', comm.lang("coDeclaration").cyToBuyResRangeEnum, 185).change(function (e) {
                ...
                kqxtyw_cyqy.showIncrementDiv();
                kqxtyw_cyqy.findResNoListAndQuota(spreadEntResNo, buyResRange, null);
            });
            ...
            $('#refincrement').click(function () {
                ...
                if (zzdqyglh.substring(0, 2) == spreadEntResNo.substring(0, 2)) {
                    kqxtyw_cyqy.findIncrement(spreadEntResNo, zzdqyglh, true);
                } else {
            ...
            $('#qyxtzcxx_save').click(function () {
                kqxtyw_cyqy.saveRegInfoStepOne();
            });
            ...
            kqxtyw_cyqy.showIncrementDiv();
            kqxtyw_cyqy.findResNoListAndQuota(data.spreadEntResNo, data.toBuyResRange, data.toEntResNo);
        },
        /**
         * 显示与隐藏积分增值信息
         */
        ...
        initData: function (data) {
            ...
            if (data.toPnodeResNo) {
                kqxtyw_cyqy.toPnodeCustId = comm.removeNull(data.toPnodeCustId);//修改前被申报增值节点父节点客户号
                kqxtyw_cyqy.toPnodeResNo = comm.removeNull(data.toPnodeResNo);//修改前被申报增值节点父节点资源
                kqxtyw_cyqy.toInodeResNo = comm.removeNull(data.toInodeResNo);//修改前被申报选择增值节点
                kqxtyw_cyqy.toInodeLorR = comm.removeNull(data.toInodeLorR);//修改前被申报选择增值节点对应区域
                ...
                this.findIncrement(data.spreadEntResNo, data.toPnodeResNo, false, chooseId);
            }
        },
        /**
         * 查询企业配额数和可用互生号列表
        ...
        findResNoListAndQuota: function (spreadEntResNo, buyResRange, resNo) {
            ...
            dataModoule.findResNoListAndQuota(params, function (res) {
                if (resNo) {
                    kqxtyw_cyqy.setHtmlData(resNo, res.data.availQuota, res.data.serCustName, res.data.serCustId);
                } else {
                    var chkType = $('input[name="chkType"]:checked').val();
                    if (chkType == '0') {
                        kqxtyw_cyqy.setHtmlData(res.data.defaultEntResNo, res.data.availQuota, res.data.serCustName, res.data.serCustId);
                    } else {
                        kqxtyw_cyqy.setHtmlData("", res.data.availQuota, res.data.serCustName, res.data.serCustId);
                    }
                }
            });
        },
            ...
            dataModoule.findIncrement({spreadEntResNo: spreadEntResNo, subResNo: subResNo}, function (res) {
                if (res.data) {
                    if (!res.data.incrementMap || $.isEmptyObject(res.data.incrementMap) || res.data.subRelation == false) {
                        ...
                        kqxtyw_cyqy.fillTable(res.data.incrementMap);
                        if (chooseId) {
        ...
        saveRegInfoStepOne: function () {
            ...
            if (toBuyResRange == "5") {//免费资源无需检查积分增值设置
                kqxtyw_cyqy.saveRegInfo([null, null, null, null]);
            } else {
                var array = this.getPnode();//获取积分增值点设置
                if (toBuyResRange == "4") {
                    if (array && array.length > 0) {
                        kqxtyw_cyqy.saveRegInfo(array);
                    } else {
        ...
        saveRegInfo: function (array) {
            ...
            //增值系统位置企业互生号
            var oldPnodeResNo = comm.removeNull(kqxtyw_cyqy.toPnodeResNo);//修改前增值系统位置企业互生号
            ...
            //增值分配点
            var oldInodeResNo = comm.removeNull(kqxtyw_cyqy.toInodeResNo);//修改前增值分配点
            var newInodeResNo = comm.removeNull(array[2]);//修改后增值分配点
            if (oldInodeResNo != newInodeResNo) {
                var oldInodeResNoText = kqxtyw_cyqy.getIncrementScore(oldInodeResNo) + "增值分配点";//修改前增值分配点
                var newInodeResNoText = kqxtyw_cyqy.getIncrementScore(newInodeResNo) + "增值分配点";//修改后增值分配点
                trs += "<tr><td class=\"view_item\">增值分配点</td><td class=\"view_text\">" + oldInodeResNoText + "</td><td class=\"view_text\">" + newInodeResNoText + "</td></tr>";
                chg.toInodeResNo = {"old": kqxtyw_cyqy.toInodeResNo, "new": newInodeResNo};
                ndata.toInodeResNo = newInodeResNo;
            }

            //增值区
            var oldInodeLorR = comm.removeNull(kqxtyw_cyqy.toInodeLorR);//修改前增值分配点
            ...
            //被申报增值节点父节点客户号
            var oldPnodeCustId = comm.removeNull(kqxtyw_cyqy.toPnodeCustId);//修改前被申报增值节点父节点客户号
            ...
            kqxtyw_cyqy.initVerificationMode();
            $('#qyxtzcxx_tj_dialog').dialog({
                width: 800,
                buttons: {
                    '确定': function () {
                        if (!kqxtyw_cyqy.validateViewMarkData().form()) {
                            return;
                        }
                        //验证双签
                        comm.verifyDoublePwd($("#doubleUserName").val(), $("#doublePassword").val(), 1, function (verifyRes) {
                            ...
                            dataModoule.saveDeclare(ndata, function (res) {
                                ...
                                kqxtyw_cyqy.toPnodeCustId = array[0];//修改后被申报增值节点父节点客户号
                                kqxtyw_cyqy.toPnodeResNo = array[1];//修改后被申报增值节点父节点资源
                                kqxtyw_cyqy.toInodeResNo = array[2];//修改后被申报选择增值节点
                                kqxtyw_cyqy.toInodeLorR = array[3];//修改后被申报选择增值节点对应区域
                                comm.alert({
