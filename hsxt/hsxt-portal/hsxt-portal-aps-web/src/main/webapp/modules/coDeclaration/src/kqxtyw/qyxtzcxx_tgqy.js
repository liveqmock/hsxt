define(['text!coDeclarationTpl/kqxtyw/qyxtzcxx_tgqy.html',
    'text!coDeclarationTpl/kqxtyw/confirm_dialog.html',
    'coDeclarationSrc/qyxz/point_choose',
    'coDeclarationDat/qyxz/qyxtzcxx_tgqy',
    'coDeclarationLan'], function (qyxtzcxx_tgqyTpl, confirm_dialogTpl, pointChoose, dataModoule) {
    var kqxtyw_tgqy = {
        applyId: null,
        odata: null,//修改前的数据
        pdata: null,//原始数据
        ...
        showPage: function (data) {
            odata = data;
            pdata = data;
            kqxtyw_tgqy.applyId = data.applyId;
            kqxtyw_tgqy.initData(data);
        },
        /**
         * 获取申请编号
         */
        ...
        initForm: function (data) {
            $('#qyxtzcxx_xgTpl').html(_.template(qyxtzcxx_tgqyTpl, data));
            ...
            comm.initSelect('#toBuyResRange', comm.lang("coDeclaration").toBuyResRangeEnum, 185).change(function (e) {
                ...
                kqxtyw_tgqy.findResNoListAndQuota(spreadEntResNo, buyResRange, null);
            });

            /**申报类型*/
            $("#toCustType").val(comm.lang("coDeclaration").tgEntDesc);
            ...
            $('#refincrement').click(function () {
                ...
                if (spreadEntResNo.substring(0, 2) == zzdqyglh.substring(0, 2)) {
                    kqxtyw_tgqy.findIncrement(spreadEntResNo, zzdqyglh, true);
                } else {
            ...
            $('#qyxtzcxx_save').click(function () {
                kqxtyw_tgqy.saveRegInfoStepOne();
            });
            ...
            kqxtyw_tgqy.findResNoListAndQuota(data.spreadEntResNo, data.toBuyResRange, data.toEntResNo);
        },
        /**
         * 初始化数据
         */
        initData: function (data) {
            ...
            if (data.toPnodeResNo) {
                kqxtyw_tgqy.toPnodeCustId = comm.removeNull(data.toPnodeCustId);//修改前被申报增值节点父节点客户号
                kqxtyw_tgqy.toPnodeResNo = comm.removeNull(data.toPnodeResNo);//修改前被申报增值节点父节点资源
                kqxtyw_tgqy.toInodeResNo = comm.removeNull(data.toInodeResNo);//修改前被申报选择增值节点
                kqxtyw_tgqy.toInodeLorR = comm.removeNull(data.toInodeLorR);//修改前被申报选择增值节点对应区域
                ...
                kqxtyw_tgqy.findIncrement(data.spreadEntResNo, data.toPnodeResNo, false, chooseId);
            }
        },
        /**
         * 查询企业配额数和可用互生号列表
        ...
        findResNoListAndQuota: function (spreadEntResNo, buyResRange, resNo) {
            ...
            dataModoule.findResNoListAndQuota(params, function (res) {
                if (resNo) {
                    kqxtyw_tgqy.setHtmlData(resNo, res.data.availQuota, res.data.serCustName, res.data.serCustId);
                } else {
                    var chkType = $('input[name="chkType"]:checked').val();
                    if (chkType == '0') {
                        kqxtyw_tgqy.setHtmlData(res.data.defaultEntResNo, res.data.availQuota, res.data.serCustName, res.data.serCustId);
                    } else {
                        kqxtyw_tgqy.setHtmlData("", res.data.availQuota, res.data.serCustName, res.data.serCustId);
                    }
                }
            });
        },
            ...
            dataModoule.findIncrement({spreadEntResNo: spreadEntResNo, subResNo: subResNo}, function (res) {
                if (res.data) {
                    if (!res.data.incrementMap || $.isEmptyObject(res.data.incrementMap) || res.data.subRelation == false) {
                        ...
                        kqxtyw_tgqy.fillTable(res.data.incrementMap);
                        if (chooseId) {
        ...
        saveRegInfo: function (array) {
            ...
            /*           var oldToBuyResRange = comm.removeNull(odata.toBuyResRange);//修改前启用资源类型
             ...
             if (oldToBuyResRange&&newToBuyResRange&&oldToBuyResRange != newToBuyResRange) {
             var oldToBuyResRangeText = comm.getNameByEnumId(oldToBuyResRange, comm.lang("coDeclaration").toBuyResRangeEnum);
             var newToBuyResRangeText = comm.getNameByEnumId(newToBuyResRange, comm.lang("coDeclaration").toBuyResRangeEnum);
            ...
            /*            var oldToSelectMode = comm.removeNull(odata.toSelectMode);//修改前顺序
             var newToSelectMode = $('input[name="chkType"]:checked').val();//修改后顺序
             if (oldToSelectMode != newToSelectMode) {
             trs += "<tr><td class=\"view_item\">选号顺序</td><td class=\"view_text\">" + comm.lang('coDeclaration').selectMode[oldToSelectMode]  + "</td><td class=\"view_text\">" + comm.lang('coDeclaration').selectMode[newToSelectMode] + "</td></tr>";
             chg.toSelectMode = {"old": oldToSelectMode, "new": newToSelectMode};
             ndata.toSelectMode = newToSelectMode;
             }

             //拟用企业互生号
             var oldToEntResNo = comm.removeNull(odata.toEntResNo);//修改前拟用企业互生号
             var newToEntResNo = $("#entResNo").val();//修改后拟用企业互生号
             if (oldToEntResNo&&newToEntResNo&&oldToEntResNo != newToEntResNo) {
            ...
            //增值系统位置企业互生号
            var oldPnodeResNo = comm.removeNull(kqxtyw_tgqy.toPnodeResNo);//修改前增值系统位置企业互生号
            var newPnodeResNo = comm.removeNull(array[1]);//修改后增值系统位置企业互生号
            if (oldPnodeResNo && newPnodeResNo && oldPnodeResNo != newPnodeResNo) {
            ...
            //增值分配点
            var oldInodeResNo = comm.removeNull(kqxtyw_tgqy.toInodeResNo);//修改前增值分配点
            var newInodeResNo = comm.removeNull(array[2]);//修改后增值分配点
            if (oldInodeResNo && newInodeResNo && oldInodeResNo != newInodeResNo) {
                var oldInodeResNoText = kqxtyw_tgqy.getIncrementScore(oldInodeResNo) + "增值分配点";//修改前增值分配点
                var newInodeResNoText = kqxtyw_tgqy.getIncrementScore(newInodeResNo) + "增值分配点";//修改后增值分配点
                trs += "<tr><td class=\"view_item\">增值分配点</td><td class=\"view_text\">" + oldInodeResNoText + "</td><td class=\"view_text\">" + newInodeResNoText + "</td></tr>";
                chg.toInodeResNo = {"old": kqxtyw_tgqy.toInodeResNo, "new": newInodeResNo};
                ndata.toInodeResNo = newInodeResNo;
            }

            //增值区
            var oldInodeLorR = comm.removeNull(kqxtyw_tgqy.toInodeLorR);//修改前增值分配点
            ...
            //被申报增值节点父节点客户号
            var oldPnodeCustId = comm.removeNull(kqxtyw_tgqy.toPnodeCustId);//修改前被申报增值节点父节点客户号
            var newPnodeCustId = comm.removeNull(array[0]);//修改后被申报增值节点父节点客户号
            if (oldPnodeCustId && newPnodeCustId && oldPnodeCustId != newPnodeCustId) {
            ...
            kqxtyw_tgqy.initVerificationMode();
            $('#qyxtzcxx_tj_dialog').dialog({
                width: 800,
                buttons: {
                    '确定': function () {
                        if (!kqxtyw_tgqy.validateViewMarkData().form()) {
                            return;
                        }
                        //验证双签
                        comm.verifyDoublePwd($("#doubleUserName").val(), $("#doublePassword").val(), 1, function (verifyRes) {
                            ...
                            dataModoule.saveDeclare(ndata, function (res) {
                                ...
                                kqxtyw_tgqy.toPnodeCustId = array[0];//修改后被申报增值节点父节点客户号
                                kqxtyw_tgqy.toPnodeResNo = array[1];//修改后被申报增值节点父节点资源
                                kqxtyw_tgqy.toInodeResNo = array[2];//修改后被申报选择增值节点
                                kqxtyw_tgqy.toInodeLorR = array[3];//修改后被申报选择增值节点对应区域
                                comm.alert({
