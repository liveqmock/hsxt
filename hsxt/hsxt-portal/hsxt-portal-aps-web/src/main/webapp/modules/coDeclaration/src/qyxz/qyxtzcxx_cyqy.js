define(['text!coDeclarationTpl/qyxz/qyxtzcxx_cyqy.html',
        'coDeclarationSrc/qyxz/point_choose',
        'coDeclarationDat/qyxz/qyxtzcxx_cyqy',
        'coDeclarationLan'], function(qyxtzcxx_cyqyTpl, pointChoose, dataModoule){
	/*
	 *推广互生号
	 */
	...
	var cyqysb = {
		...
		initForm: function (data) {
			...
			$('#busibox').html(_.template(qyxtzcxx_cyqyTpl, data));
			...
			comm.initSelect('#toBuyResRange', comm.lang("coDeclaration").cyToBuyResRangeEnum, 185).change(function (e) {
				...
				cyqysb.showIncrementDiv();
				cyqysb.findResNoListAndQuota(spreadEntResNo, buyResRange, null);
			});
			...
			$('#refincrement').click(function () {
				...
				if (zzdqyglh.substring(0,2)==spreadEntResNo.substring(0,2)) {
					cyqysb.findIncrement(spreadEntResNo,zzdqyglh, true);
				}else{
			...
			$('#qyxtzcxx_save').click(function () {
				cyqysb.saveRegInfoStepOne(false);
			});

			//下一步 
			$('#qyxtzcxx_next').click(function () {
				cyqysb.saveRegInfoStepOne(true);
			});
			...
			if (data) {
				...
				cyqysb.showIncrementDiv();
				cyqysb.initPCList(data.provinceNo, data.cityNo, data.spreadEntResNo);
				cyqysb.findResNoListAndQuota(data.spreadEntResNo, data.toBuyResRange, data.toEntResNo);
			...
			$('#spreadEntResNo_refresh').bind('click',function () {
				var regex = /^(([1-9]\d){1}|(\d[1-9]){1})([1-9]\d{2}|\d{2}[1-9]|\d[1-9]\d)([0]{6})$/;
				var spResNo = $.trim($('#spreadEntResNo').val());
				if(spResNo&&regex.test(spResNo)) {
					dataModoule.findResNoDetail({spreadEntResNo:spResNo},function (result) {
						if(result.data) {
							cyqysb.initPCList(result.data.provinceNo, result.data.cityNo);
							//经营类型 0：普通 1：连锁企业
							if(result.data.businessType==0) {
								$('#province_slt').selectEnabled(false);
								$('#city_slt').selectEnabled(false);
							...
							cyqysb.findIncrement(spResNo,spResNo, false);
							var buyResRange = $("#toBuyResRange").attr('optionValue');
							if(buyResRange) {
								cyqysb.findResNoListAndQuota(spResNo, buyResRange, null);
							}
							$('#refincrement').attr('disabled', false);
							$('#zzdqyglh').val('');
						}else{
		...
		showIncrementDiv: function () {
			...
			if (toBuyResRange == "5") {
				...
				$("#comboxDiv").show();
				var spreadEntResNo = $("#spreadEntResNo").val();
				if(!spreadEntResNo)return;
				var zzdqyglh = $.trim($("#zzdqyglh").val())||spreadEntResNo;
				if (zzdqyglh.substring(0,2)==spreadEntResNo.substring(0,2)) {
					cyqysb.findIncrement(spreadEntResNo,zzdqyglh, false);
				}
			}
		},
		/**
			...
			if (applyId == null) {
				...
				dataModoule.findDeclareByApplyId({applyId: applyId}, function (res) {
					...
					if (res.data.toPnodeResNo) {
						...
						cyqysb.findIncrement(res.data.spreadEntResNo,res.data.toPnodeResNo, false, chooseId);
					}
				});
			}
		},
		...
		findResNoListAndQuota: function (spreadEntResNo, buyResRange, resNo) {
			...
			dataModoule.findResNoListAndQuota(params, function (res) {
				if (resNo) {
					cyqysb.setHtmlData(resNo, res.data.availQuota, res.data.serCustName, res.data.serCustId);
				} else {
					var chkType = $('input[name="chkType"]:checked').val();
					if (chkType == '0') {
						cyqysb.setHtmlData(res.data.defaultEntResNo, res.data.availQuota, res.data.serCustName, res.data.serCustId);
					} else {
						cyqysb.setHtmlData("", res.data.availQuota, res.data.serCustName, res.data.serCustId);
					}
				}
			});
		},
			...
			dataModoule.findIncrement({spreadEntResNo:spreadEntResNo,subResNo: subResNo}, function (res) {
				if (res.data) {
					if(!res.data.incrementMap||$.isEmptyObject(res.data.incrementMap)||res.data.subRelation==false){
						...
						cyqysb.fillTable(res.data.incrementMap);
						if (isAlert) {
		...
		saveRegInfoStepOne: function (autoNext) {
			...
			if (parseInt(availQuota) == 0) {
				comm.error_alert(comm.lang("coDeclaration").nocyPointNo);
				return;
			}
			...
			if (toBuyResRange == "5") {//免费资源无需检查积分增值设置
				cyqysb.saveRegInfo(autoNext, [null, null, null, null]);
			} else {
				var array = this.getPnode();//获取积分增值点设置
				if (toBuyResRange == "4" && array &&array.length>0) {
					cyqysb.saveRegInfo(autoNext, array);
				}
			}
		},
		/**
		...
		saveRegInfo: function (autoNext, array) {
			...
			dataModoule.checkValidEntResNo(params, function (result) {
				if(result.data&&result.data==true)  {//可用
					...
					cyqysb.iConfirm(comm.lang('common')[12544],function () {
						dataModoule.saveDeclare(params, function (res) {
							cyqysb.saveStep(1, res.data);
							if (autoNext) {
								$('#qygsdjxx').click();
