define(['text!coDeclarationTpl/qybb/yybbsh.html',
    'text!coDeclarationTpl/qybb/yybbsh/ckxq_dialog_1.html',
    'text!coDeclarationTpl/qybb/yybbsh/ckxq_dialog_2.html',
    'text!coDeclarationTpl/qybb/yybbsh/check_dialog.html',
    'text!coDeclarationTpl/qybb/same_item_dialog.html',
    'coDeclarationDat/qybb/qybb',
    'coDeclarationDat/qybb/yybbsh',
    'coDeclarationLan'], function (yybbshTpl, ckxq_dialog_1Tpl, ckxq_dialog_2Tpl, check_dialogTpl,same_item_dialogTpl, qybbAjax, dataModoule) {
    var aps_yybbsh = {
        showPage: function () {
            $('#busibox').html(_.template(yybbshTpl));
...
//			aps_yybbsh.initData();
        ...
        initData: function () {
            ...
            dataModoule.findDisagreedFilingList(params, this.det, this.del, this.add);
        },
        /**
         * 显示相同项
         */
            ...
            if (colIndex == 1) {
                ...
                if (record['existSameItem']) {
                    return $('<a class="existSameItem">查看</a>').click(function () {
                        aps_yybbsh.querySameItem(record);
                    }).bind(this);
                } else {
                    return comm.lang("coDeclaration").no;
                }
            } else if (colIndex == 6) {
                return comm.getNameByEnumId(record['status'], comm.lang("coDeclaration").raiseStatusEnum);
            } else if (colIndex == 8) {
                /*return  $('<a data-type="detail" >查看</a>').click(function(e) {
                 var status = record['status'];
                 var tpl = (status == 1)?ckxq_dialog_1Tpl:ckxq_dialog_2Tpl;
                 //查看详情
                 ...
                 $('#detail_dialog').dialog({width:1000,title:'异议报备详情',buttons:{'确定':function(){$(this).dialog('destroy')}}});
                 });
                 });
                 */
            }
            ...
            if (colIndex == 7) {
                ...
                if (status == 1 || status == 2 || status == 3 || status == 4) {
                    return $('<a data-type="update" >修改</a>').click(function (e) {
                        comm.liActive_add($('#yybbxg'));
                        require(['coDeclarationSrc/qybb/sub_tab'], function (tab) {
                            tab.showPage(record['applyId'], "yybbsh");
                        });
                    });
                }
            }
