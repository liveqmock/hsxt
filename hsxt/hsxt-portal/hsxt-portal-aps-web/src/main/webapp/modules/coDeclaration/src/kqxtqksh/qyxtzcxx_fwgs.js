define(['text!coDeclarationTpl/kqxtqksh/qyxtzcxx_fwgs.html',
	...
	var kqxtqksh_fwgs = {
		...
		showPage: function (data) {
			odata = data;
			pdata = data;
			kqxtqksh_fwgs.applyId = data.applyId;
			kqxtqksh_fwgs.initData(data);
		},
		/**
		 * 获取申请编号
		 */
		getApplyId: function () {
			return kqxtqksh_fwgs.applyId;
		},
		/**
		 * 初始化表单
		 */
		initForm: function (data) {
			...
			$('#refincrement').click(function () {
				...
				if (toMResNo.substring(0, 2) == spreadEntResNo.substring(0, 2)) {
					kqxtqksh_fwgs.findIncrement(spreadEntResNo, zzdqyglh || spreadEntResNo, true);
				} else {
					if (zzdqyglh) {
						kqxtqksh_fwgs.findIncrement(toMResNo, zzdqyglh, true);
					} else {
			...
			$('#qyxtzcxx_save').click(function () {
				kqxtqksh_fwgs.saveRegInfoStepOne();
			});
			...
			$('#spreadEntResNo').blur(function () {
				...
				if (serRegex.test(spreadEntResNo) || platRegex.test(spreadEntResNo)) {
					kqxtqksh_fwgs.findEntInfo(spreadEntResNo);
				} else {
			...
			kqxtqksh_fwgs.findManageEntAndQuota(data.countryNo, data.provinceNo, data.cityNo);
			kqxtqksh_fwgs.findEntInfo(comm.removeNull(data.spreadEntResNo));
		},
		/**
		 * 依据互生号查询企业信息
		...
		initData: function (data) {
			...
			if (data.toPnodeResNo) {
				kqxtqksh_fwgs.toPnodeCustId = comm.removeNull(data.toPnodeCustId);//修改前被申报增值节点父节点客户号
				kqxtqksh_fwgs.toPnodeResNo = comm.removeNull(data.toPnodeResNo);//修改前被申报增值节点父节点资源
				kqxtqksh_fwgs.toInodeResNo = comm.removeNull(data.toInodeResNo);//修改前被申报选择增值节点
				kqxtqksh_fwgs.toInodeLorR = comm.removeNull(data.toInodeLorR);//修改前被申报选择增值节点对应区域
				...
				if (serRegex.test(spreadEntResNo) && toMResNo.substring(0, 2) == spreadEntResNo.substring(0, 2)) {
					...
					kqxtqksh_fwgs.findIncrement(spreadEntResNo, toPnodeResNo, false, chooseId);
				} else if (toMResNo != toPnodeResNo) {
					kqxtqksh_fwgs.findIncrement(toMResNo, toPnodeResNo, false, chooseId);
				}
			}
		},
		/**
		...
		findManageEntAndQuota: function (countryNo, provinceNo, cityNo) {
			...
			dataModoule.findManageEntAndQuota(params, function (res) {
				if (res.data) {
					...
					kqxtqksh_fwgs.setHtmlData(entResNo, res.data.quota, entName);
				}
			});
		},
		/**
			...
			dataModoule.findIncrement({spreadEntResNo:spreadEntResNo,subResNo: subResNo}, function (res) {
				if (res.data) {
					if (!res.data.incrementMap || $.isEmptyObject(res.data.incrementMap) || res.data.subRelation == false) {
						...
						kqxtqksh_fwgs.fillTable(res.data.incrementMap);
						if (chooseId) {
		...
		saveRegInfoStepOne: function () {
			...
			if (array == null) {
				...
				kqxtqksh_fwgs.saveRegInfo([null, null, null, null]);
			} else {
				kqxtqksh_fwgs.saveRegInfo(array);
			}
		},
		/**
		 * 保存注册信息
		...
		saveRegInfo: function (array) {
			...
			if (comm.removeNull(odata.toBusinessType) != newToBusinessType) {
				...
				trs += "<tr><td class=\"view_item\">经营类型</td><td class=\"view_text\">" + newText + "</td><td class=\"view_text\">" + oldText + "</td></tr>";
				chg.toBusinessType = {"old": comm.removeNull(odata.toBusinessType), "new": newToBusinessType};
				ndata.toBusinessType = newToBusinessType;
			}

			//增值系统位置企业互生号
			var oldPnodeResNo = comm.removeNull(kqxtqksh_fwgs.toPnodeResNo);//修改前增值系统位置企业互生号
			...
			//增值分配点
			var oldInodeResNo = comm.removeNull(kqxtqksh_fwgs.toInodeResNo);//修改前增值分配点
			var newInodeResNo = comm.removeNull(array[2]);//修改后增值分配点
			if (oldInodeResNo != newInodeResNo) {
				var oldInodeResNoText = kqxtqksh_fwgs.getIncrementScore(oldInodeResNo) + "增值分配点";//修改前增值分配点
				var newInodeResNoText = kqxtqksh_fwgs.getIncrementScore(newInodeResNo) + "增值分配点";//修改后增值分配点
				trs += "<tr><td class=\"view_item\">增值分配点</td><td class=\"view_text\">" + oldInodeResNoText + "</td><td class=\"view_text\">" + newInodeResNoText + "</td></tr>";
				chg.toInodeResNo = {"old": kqxtqksh_fwgs.toInodeResNo, "new": newInodeResNo};
				ndata.toInodeResNo = newInodeResNo;
			}

			//增值区
			var oldInodeLorR = comm.removeNull(kqxtqksh_fwgs.toInodeLorR);//修改前增值分配点
			...
			//被申报增值节点父节点客户号
			var oldPnodeCustId = comm.removeNull(kqxtqksh_fwgs.toPnodeCustId);//修改前被申报增值节点父节点客户号
			...
			kqxtqksh_fwgs.initVerificationMode();
			$('#qyxtzcxx_tj_dialog').dialog({
				width: 800,
				buttons: {
					'确定': function () {
						if (!kqxtqksh_fwgs.validateViewMarkData().form()) {
							return;
						}
						//验证双签
						comm.verifyDoublePwd($("#doubleUserName").val(), $("#doublePassword").val(), 1, function (verifyRes) {
							...
							dataModoule.saveDeclare(ndata, function (res) {
								...
								kqxtqksh_fwgs.toPnodeCustId = array[0];//修改后被申报增值节点父节点客户号
								kqxtqksh_fwgs.toPnodeResNo = array[1];//修改后被申报增值节点父节点资源
								kqxtqksh_fwgs.toInodeResNo = array[2];//修改后被申报选择增值节点
								kqxtqksh_fwgs.toInodeLorR = array[3];//修改后被申报选择增值节点对应区域
								comm.alert({
