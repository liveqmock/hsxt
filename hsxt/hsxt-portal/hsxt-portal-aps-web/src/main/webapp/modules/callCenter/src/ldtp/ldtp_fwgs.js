                            ...
                            detail: function (record, rowIndex, colIndex, options) {
                                ...
                                if(3==colIndex){
                                	cacheUtil.syncGetRegionByCode(record.countryNo, record.provinceNo, record.cityNo, "", function (resdata) {
                                		$("#searchTable_fwgs_yhzhxx_ldtp   tr:eq("+(rowIndex+1)+") td:eq(3)").html(resdata);
                                    });
                                }
                                if(5==colIndex){
...
//                    $("#ldtp_s_xtdjxx tr:eq(3) td:odd:eq(0)").text(comm.isEmpty(data.baseInfo.entNameEn) ? "" : data.baseInfo.entNameEn);//企业英文名称
                    cacheUtil.syncGetRegionByCode(data.baseInfo.countryCode, data.baseInfo.provinceCode, data.baseInfo.cityCode, "", function (resdata) {
                        $("#ldtp_s_xtdjxx tr:eq(3) td:odd:eq(0)").html(resdata);//所属地区
                    });
                    $("#ldtp_s_xtdjxx tr:eq(4) td:odd:eq(0)").html(data.baseInfo.countryCode == "156" ? "人民币" : "未知");//币种
                    $("#ldtp_s_xtdjxx tr:eq(5) td:odd:eq(0)").html(data.extendInfo.businessType== 1? '连锁企业' : '普通企业');//经营类型
                    //工商登记信息
...
//                    $("#ldtp_s_gsdjxx tr:eq(2) td:odd:eq(1)").html(data.mainInfo.creNo);//法人代表证件号码
                    $("#ldtp_s_gsdjxx tr:eq(2) td:odd:eq(0)").html(data.baseInfo.nature);//企业类型
                    $("#ldtp_s_gsdjxx tr:eq(2) td:odd:eq(1)").html(data.mainInfo.busiLicenseNo);//营业执照注册号
...
//                    $("#ldtp_s_gsdjxx tr:eq(4) td:odd:eq(1)").html(data.mainInfo.taxNo);//纳税人识别号
                    $("#ldtp_s_gsdjxx tr:eq(3) td:odd:eq(0)").html(data.extendInfo.buildDate);//成立日期
                    $("#ldtp_s_gsdjxx tr:eq(3) td:odd:eq(1)").html(data.extendInfo.endDate);//营业期限
                    $("#ldtp_s_gsdjxx tr:eq(4) td:odd:eq(0)").html(data.extendInfo.businessScope);//经营范围
                    $("#ldtp_fwgs_busiLicenseImg").attr("src", comm.getFsServerUrl(data.extendInfo.busiLicenseImg));//营业执照扫描件
                    ...
                    //委托授权书
                    $("#ldtp_fwgs_authProxyFile").attr("src", comm.getFsServerUrl(data.mainInfo.authProxyFile));
                    //银行账户信息--专用接口
                    //实名认证信息
                    ...
                    $("#ldtp_s_xq_xtdjxx tr:eq(1) td:odd:eq(1)").html(data.extendInfo.businessType== 1? '连锁企业' : '普通企业');//经营类型
                    cacheUtil.syncGetRegionByCode(data.baseInfo.countryCode, data.baseInfo.provinceCode, data.baseInfo.cityCode, "", function (resdata) {
...
//                    $("#ldtp_s_xq_gsdjxx tr:eq(1) td:odd:eq(1)").html(data.mainInfo.contactPhone);//联系电话
                    $("#ldtp_s_xq_gsdjxx tr:eq(2) td:odd:eq(0)").html(data.baseInfo.nature);//企业类型
                    $("#ldtp_s_xq_gsdjxx tr:eq(2) td:odd:eq(1)").html(data.mainInfo.busiLicenseNo);//营业执照注册号
...
//                    $("#ldtp_s_xq_gsdjxx tr:eq(3) td:odd:eq(1)").html(data.mainInfo.taxNo);//纳税人识别号
                    $("#ldtp_s_xq_gsdjxx tr:eq(3) td:odd:eq(0)").html(data.extendInfo.buildDate);//成立日期
                    $("#ldtp_s_xq_gsdjxx tr:eq(3) td:odd:eq(1)").html(data.extendInfo.endDate);//营业期限
                    $("#ldtp_s_xq_gsdjxx tr:eq(4) td:odd:eq(0)").html(data.extendInfo.businessScope);//经营范围
                    $("#ldtp_fwgs_xq_busiLicenseImg").attr("src", comm.getFsServerUrl(data.extendInfo.busiLicenseImg));//营业执照扫描件
                    //联系信息
...
//                    $("#ldtp_s_xq_lxxx tr:eq(2) td:odd:eq(0)").html(data.baseInfo.webSite); //企业网址
                    $("#ldtp_s_xq_lxxx tr:eq(2) td:odd:eq(0)").html(data.baseInfo.contactEmail); //企业安全邮箱
                    //委托授权书
                    $("#ldtp_fwgs_xq_authProxyFile").attr("src", comm.getFsServerUrl(data.mainInfo.authProxyFile));
            ...
            callCenter.getAccountBalance(param, function (request) {
                if (request.retCode == 22000) {
                    ...
                    if(comm.isNotEmpty(balance)){
                        ...
                        $("#ldtp_s_pointBalance").html(comm.formatMoneyNumberAps(pointBalance));
                        //可用积分数
                        var pointBalance_usable = comm.isNotEmpty(balance[10110]) ? (balance[10110].accBalance - 10) : 0;
                        $("#ldtp_s_pointBalance_usable").html(comm.formatMoneyNumberAps(pointBalance_usable));
                        //货币账户余额
                        var cashBalance = comm.isNotEmpty(balance[30110]) ? balance[30110].accBalance : 0;
                        $("#ldtp_s_cashBalance").html(comm.formatMoneyNumberAps(cashBalance));
                        //流通币
                        var hsb_ltb = comm.isNotEmpty(balance[20110]) ? balance[20110].accBalance : 0;
                        $("#ldtp_s_hsb_ltb").html(comm.formatMoneyNumberAps(hsb_ltb));
                        //慈善救助金
                        var hsb_csjz = comm.isNotEmpty(balance[20130]) ? balance[20130].accBalance : 0;
                        $("#ldtp_s_hsb_csjz").html(comm.formatMoneyNumberAps(hsb_csjz));
                        //积分税收金额总数
                        var point_tax = comm.isNotEmpty(balance[10510]) ? balance[10510].accBalance : 0;
                        $("#ldtp_s_point_tax").html(comm.formatMoneyNumberAps(point_tax));
                        //互生币税收金额总数
                        var hsb_tax = comm.isNotEmpty(balance[20610]) ? balance[20610].accBalance : 0;
                        $("#ldtp_s_hsb_tax").html(comm.formatMoneyNumberAps(hsb_tax));
                        //货币(人民币)税收金额总数
                        var cash_tax = comm.isNotEmpty(balance[30310]) ? balance[30310].accBalance : 0;
                        $("#ldtp_s_cash_tax").html(comm.formatMoneyNumberAps(cash_tax));
                        //积分投资总数
                        var invest_sum = comm.isNotEmpty(balance[10410]) ? balance[10410].accBalance : 0;
                        $("#ldtp_s_invest_sum").html(comm.formatMoneyNumberAps(invest_sum));
                        //本年度投资分红互生币
                        $("#ldtp_s_income_sum").html(comm.formatMoneyNumberAps(invest_income_sum));


                        /******************详单账户tab******************/
                        /**积分账户**/
                        $("#ldtp_s_xq_zhxx #company_point_balance").html(comm.formatMoneyNumberAps(pointBalance));   //积分账户余数
                        $("#ldtp_s_xq_zhxx #company_point_balance_usable").html(comm.formatMoneyNumberAps(pointBalance_usable));//可用积分数
                        $("#ldtp_s_xq_zhxx #company_point_yestday").html(comm.formatMoneyNumberAps(point_yestday));//昨日积分数
                        /** 投资账户 **/
                        $("#ldtp_s_xq_zhxx #company_invest_sum").html(comm.formatMoneyNumberAps(invest_sum));//积分投资总数
                        $("#ldtp_s_xq_zhxx #company_invest_income").html(comm.formatMoneyNumberAps(invest_income_sum));//本年度投资分红互生币
                        $("#ldtp_s_xq_zhxx #company_invest_income_ltb").html(comm.formatMoneyNumberAps(invest_income_ltb));//本年度流通币
                        $("#ldtp_s_xq_zhxx #company_invest_income_csjz").html(comm.formatMoneyNumberAps(invest_income_csjz));//本年度慈善救助
                        /** 货币账户 **/
                        $("#ldtp_s_xq_zhxx #company_currency_balance").html(comm.formatMoneyNumberAps(cashBalance));//货币账户余额
                        /** 互生币账户 **/
                        $("#ldtp_s_xq_zhxx #company_hsb_ltb").html(comm.formatMoneyNumberAps(hsb_ltb));//流通币
                        $("#ldtp_s_xq_zhxx #company_hsb_csjz").html(comm.formatMoneyNumberAps(hsb_csjz));//慈善救助金
                        /** 城市税收对接账户 **/
                        $("#ldtp_s_xq_zhxx #company_point_tax").html(comm.formatMoneyNumberAps(point_tax)).attr("style", "margin-right:20px");//积分税收金额总数
                        $("#ldtp_s_xq_zhxx #company_hsb_tax").html(comm.formatMoneyNumberAps(hsb_tax)).attr("style", "margin-right:20px");//互生币税收金额总数
                        $("#ldtp_s_xq_zhxx #company_currency_tax").html(comm.formatMoneyNumberAps(cash_tax)).attr("style", "margin-right:20px");//货币(人民币)税收金额总数
                    }
                }
            });
        },
          ...
          callCenter.searchEntAllInfo(param, function (request) {
           ...
           if (request.retCode == 22000) {
        	  var data = request.data;
         	  ...
         	  //直接看到的是截取过的,防止服务公司名过长滚动条出现时内容会挤压变形
