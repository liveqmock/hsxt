define(['text!serviceMsgManageTpl/xxmb/yjmb/yjmb.html',
		'text!serviceMsgManageTpl/xxmb/yjmb/yjmb_xz.html',
		'text!serviceMsgManageTpl/xxmb/yjmb/yjmb_xg.html',
		'text!serviceMsgManageTpl/xxmb/yjmb/yjmb_ck.html',
		'serviceMsgManageDat/serviceMsgManage',
        'serviceMsgManageLan'
], function(yjmbTpl, yjmb_xzTpl, yjmb_xgTpl, yjmb_ckTpl, dataModoule){
	return {
		yjmb_self : null,
		showPage : function(){
			yjmb_self = this;
		 ...
		 */
		initForm : function(){
			$('#busibox').html(_.template(yjmbTpl));
			/** 查询事件 */
		...
		xiuGai : function(record){
			$('#xzmbTpl').html(yjmb_xzTpl);
			yjmb_self.showTpl($('#xzmbTpl'));
			...
			comm.initSelect('#custType', comm.lang("serviceMsgManage").custTypes, 185, record.custType, null).change(function(){
				yjmb_self.initChange($(this).attr('optionValue'), "");
				yjmb_self.initBusTypeSelect("");
			});
			yjmb_self.initChange(record.custType, comm.removeNull(record.buyResType));
			yjmb_self.initBusTypeSelect(record.bizType);
			...
			$('#xzmb_save_btn').click(function(){
				yjmb_self.saveData("#save_form", record.tempId);
			});
			
			$('#xzmb_back_btn').click(function(){
				yjmb_self.showTpl($('#yjmbTpl'));
		...
		xinZhen : function(){
			...
			$('#xzmb_back_btn').click(function(){
				that.showTpl($('#yjmbTpl'));
				comm.liActive($('#yjmb'), '#xzmb, #xgmb');	
			});
		},
		showTpl : function(tplObj){
			$('#yjmbTpl, #xzmbTpl, #xgmbTpl, #ck_dialog').addClass('none');
		...
		saveData : function(formId, tempId){
			if(!yjmb_self.validateData(formId).form()){
				return;
			}
			var params = {};
		...
		initBusTypeSelect : function(val){
			...
			var yjTypes = comm.lang("serviceMsgManage").yjBusTypes[custType+"_"+buyResType];
			var yjCustTypes = {};
			if(yjTypes){
				for(var i=0; i<yjTypes.length; i++){
					yjCustTypes[yjTypes[i]] = comm.lang("serviceMsgManage").busTypes[yjTypes[i]];
				}
			}
			comm.initSelect('#bizType', yjCustTypes, 185, val, {name:'', value:''});
		}
	}	
});