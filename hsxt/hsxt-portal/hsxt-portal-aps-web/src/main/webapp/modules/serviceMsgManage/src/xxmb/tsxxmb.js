define(['text!serviceMsgManageTpl/xxmb/tsxxmb/tsxxmb.html',
		'text!serviceMsgManageTpl/xxmb/tsxxmb/tsxxmb_xz.html',
		'text!serviceMsgManageTpl/xxmb/tsxxmb/tsxxmb_xg.html',
		'text!serviceMsgManageTpl/xxmb/tsxxmb/tsxxmb_ck.html',
		'serviceMsgManageDat/serviceMsgManage',
        'serviceMsgManageLan'
], function(tsxxmbTpl, tsxxmb_xzTpl, tsxxmb_xgTpl, tsxxmb_ckTpl, dataModoule){
	return {
		tsxxmb_self : null,
		showPage : function(){
			tsxxmb_self = this;
			tsxxmb_self.initForm();
		},
		/**
		 * 初始化界面
		 */
		initForm : function(){
			$('#busibox').html(_.template(tsxxmbTpl));
			/** 查询事件 */
			$("#queryBtn").click(function(){
				tsxxmb_self.initData();
			});
		...
		xiuGai : function(record){
			$('#xzmbTpl').html(tsxxmb_xzTpl);
			tsxxmb_self.showTpl($('#xzmbTpl'));
			comm.liActive_add($('#xgmb'));
			
			comm.initSelect('#custType', comm.lang("serviceMsgManage").tsCustTypes, 185, record.custType, null).change(function(){
				tsxxmb_self.initChange($(this).attr('optionValue'), "");
				tsxxmb_self.initBusTypeSelect("");
			});
			tsxxmb_self.initChange(record.custType, comm.removeNull(record.buyResType));
			tsxxmb_self.initBusTypeSelect(record.bizType);
			...
			$('#xzmb_save_btn').click(function(){
				tsxxmb_self.saveData("#save_form", record.tempId);
			});
			
			$('#xzmb_back_btn').click(function(){
				tsxxmb_self.showTpl($('#tsxxmbTpl'));
		...
		xinZhen : function(){
			...
			comm.initSelect('#custType', comm.lang("serviceMsgManage").tsCustTypes, 185, null, null).change(function(){
			...
			$('#xzmb_back_btn').click(function(){
				that.showTpl($('#tsxxmbTpl'));
				comm.liActive($('#tsxxmb'), '#xzmb, #xgmb');	
			});
		},
		showTpl : function(tplObj){
			$('#tsxxmbTpl, #xzmbTpl, #xgmbTpl, #ck_dialog').addClass('none');
		...
		saveData : function(formId, tempId){
			if(!tsxxmb_self.validateData(formId).form()){
				return;
			}
			var params = {};
		...
		initBusTypeSelect : function(val){
			...
			var tsTypes = comm.lang("serviceMsgManage").tsBusTypes[custType+"_"+buyResType];
			var tsCustTypes = {};
			if(tsTypes){
				for(var i=0; i<tsTypes.length; i++){
					tsCustTypes[tsTypes[i]] = comm.lang("serviceMsgManage").busTypes[tsTypes[i]];
				}
			}
			comm.initSelect('#bizType', tsCustTypes, 185, val, {name:'', value:''});
		}
	}	
});