define(["commonSrc/frame/chat/strophe",
	"commSrc/frame/chat/mcsChat"],function(strophe, mcsChat){
	...
	$.hsChat.groupIds = {};//存入分组及用户
	$.hsChat.groupIdc = {};//存入分组未读消息数量
	$.hsChat.userList = {};//存入用户不分组
...
//		xmppProtocol : comm.removeNull(comm.domainList.xmppProtocol)==""?"ws":comm.domainList.xmppProtocol,
	...
	$.hsChat.initEvent = function(){
		...
		$("#companyChatBox").keydown(function(e){
     ...
     */
	 function onMessage(msg) {
		...
		if(jsonobj.msg_type == 2){
			...
			//截取WEB端移动端公众号
			var formUserId = from.split("@")[0].replace("w_e_", "").replace("m_e_", "");
			//只处理通讯录中的用户信息
			if($.hsChat.userList[formUserId]){
	...
	$.hsChat.dealRecvMessage = function(formUserId, content, isCurrent, msgCode, msgid){
		...
		var groupIds = $.hsChat.userList[formUserId].groupIds;
		$.each(groupIds, function(key, groupId){
			var userList = $.hsChat.groupIds[groupId];
			var tempList = [null];
			$.each(userList, function(key, user){
				if(user.optCustId == formUserId){
					tempList[0] = $.hsChat.userList[formUserId];
					tempList[0].lastTime = $.hsChat.getRecvDate(msgid);
					tempList[0].msgContent = (msgCode == "10")?"[图片]":content;
				}else{
					tempList.push($.hsChat.userList[user.optCustId]);
				}
			});
			$.hsChat.groupIds[groupId] = tempList;
			$.hsChat.search(tempList, groupId);
		});
	}
	...
	$.hsChat.dealSendMessage= function(content, toUserId, msgCode){
		...
		content = content.replace(/\\r\\n/igm, ' ');
		var groupIds = $.hsChat.userList[toUserId].groupIds;
		$.each(groupIds, function(key, groupId){
			var userList = $.hsChat.groupIds[groupId];
			var tempList = [null];
			$.each(userList, function(key, user){
				if(user.optCustId == toUserId){
					tempList[0] = $.hsChat.userList[toUserId];
...
//		$.hsChat.conn = new Strophe.Connection($.hsChat.defaults.xmpp, {protocol: $.hsChat.defaults.xmppProtocol});
	...
	//加载搜索结果
	$.hsChat.search = function(userList, groupId){
		$("#qytxl_"+groupId).html("");
		$.each(userList, function(key, user){
			var liId = "li_"+groupId+"_"+user.optCustId;
			var ulliId = "count_"+groupId+"_"+user.optCustId;
			var dataId = user.optCustId+"_"+$.trim(user.optWorkNo);
			var userLi="<li id="+liId+" data-id="+dataId+"><dl>";
			userLi+="<dt class=\"pr\"><img src=\"resources/img/ico-chat4.jpg\"/>";
			if(user.msgCount){
				if(user.msgCount > 99){
					userLi+="<span id="+ulliId+" class=\"count pa\" style=\"right:-5px;top:-7px;font-size:12px;\">99+</span>";
				}else{
					userLi+="<span id="+ulliId+" class=\"count pa\" style=\"right:-5px;top:-7px;font-size:12px;\">"+user.msgCount+"</span>";
				}
			}else{
				userLi+="<span id="+ulliId+" class=\"count pa\" style=\"display:none;\"></span>";
			}
			userLi+="</dt>";
			userLi+="<dd class=\"name\"><span class=\"hd-name-limit\">"+user.operatorName+"</span></dd>";
			if(user.msgContent){
			...
			userLi+="</dl></li>";
			$("#qytxl_"+groupId).append(userLi);
		});
		$.hsChat.bindChat(groupId);
	}
	
	//企业通讯录聊天窗口操作事件（普通）
	$.hsChat.bindChat = function(groupId){
		$("#qytxl_"+groupId).find('li').click(function(){
			var user = {};
			...
			mcsChat.showChatBox(user);
			var unReadMsgCount = $.hsChat.getMessageCount(user.userCustId);
			$.hsChat.showHistory(user, unReadMsgCount);
			$.hsChat.subMessageCount(user.userCustId, $.hsChat.getMessageCount(user.userCustId));
		});
	}
	
	//企业通讯录聊天窗口操作事件（搜索）
	$.hsChat.chatClick = function(custId){
		if(!$.hsChat.userList[custId]){
			return;
		}
		var groupIds = $.hsChat.userList[custId].groupIds;
		var liObj = $("#"+"li_"+groupIds[0]+"_"+custId);
		var user = {};
	...
	//查找聊天记录
	$.hsChat.showHistory = function(user, unReadMsgCount){
		var pageSize = 50;
		...
		comm.Request({url:"queryMessageRecordList", domain:"xmppBservice"},{
			...
			success:function(response){
				if(response.retCode == 200){
					...
					//清除所有该用户未读消息
					var groupIds = $.hsChat.userList[user.userCustId].groupIds;
					$.each(groupIds, function(key, groupId){
						$("#count_"+groupId+"_"+user.userCustId).hide("");
					});
				}else{
					$.hsChat.lastMsgTime = $.hsChat.getCurrTime();
	...
	//聊天--获取未读消息数量
	$.hsChat.getMessageCount = function(custId){
		return $.hsChat.userList[custId].msgCount;
	};
	
	//聊天--增加未读消息数量
	$.hsChat.addMessageCount = function(custId){
		$.hsChat.userList[custId].msgCount = $.hsChat.userList[custId].msgCount+1;
		$.each($.hsChat.userList[custId].groupIds, function(key, groupId){
			$.hsChat.groupIdc["total"] = $.hsChat.groupIdc["total"]+1;
			$.hsChat.groupIdc[groupId] = $.hsChat.groupIdc[groupId]+1;
			if($.hsChat.userList[custId].msgCount > 99){
				$("#count_"+groupId).html("99+");
			}else{
				$("#count_"+groupId).html($.hsChat.groupIdc[groupId]);
			}
			$("#count_"+groupId).show();
		});
		if($.hsChat.groupIdc["total"] > 0){
			$("#li_chat").html("消息("+($.hsChat.groupIdc["total"]>99?"99+":$.hsChat.groupIdc["total"])+")");
		}else{
			$("#li_chat").html("消息");
		}
	};
	...
	//聊天--减少未读消息数量
	$.hsChat.subMessageCount = function(custId, msgCount){
		$.hsChat.userList[custId].msgCount = $.hsChat.substruction($.hsChat.userList[custId].msgCount, msgCount);
		$.each($.hsChat.userList[custId].groupIds, function(key, groupId){
			$.hsChat.groupIdc["total"] = $.hsChat.substruction($.hsChat.groupIdc["total"], msgCount);
			$.hsChat.groupIdc[groupId] = $.hsChat.substruction($.hsChat.groupIdc[groupId], msgCount);
			if($.hsChat.userList[custId].msgCount > 99){
				$("#count_"+groupId).html("99+");
			}else{
				$("#count_"+groupId).html($.hsChat.groupIdc[groupId]);
			}
			if($.hsChat.groupIdc[groupId] == 0){
				$("#count_"+groupId).hide();
			}
		});
		if($.hsChat.groupIdc["total"] > 0){
	...
	//聊天--计算未读消息
	$.hsChat.substruction = function(num1, num2){
		var num = num1 - num2;
		if(num > 0){
			return num;
		}
