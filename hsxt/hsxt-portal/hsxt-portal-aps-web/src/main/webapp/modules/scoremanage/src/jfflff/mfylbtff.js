define([
"text!scoremanageTpl/jfflff/mfylbtff.html",
...
'scoremanageLan'
],function(mfylbtffTpl,ffTpl,sq_dialogTpl,jfflff_gq_dialog,pointWelfare){
 
	var self = {
			showPage : function(){
				$('#busibox').html(_.template(mfylbtffTpl));	
				/** 查询事件 */
		...
		pageQuery:function(){
			...
			//查询参数
			var queryParam={
							"search_welfareType":"1",	//int	否	福利类型 0 意外伤害 1 免费医疗 2 他人身故
		...
		detail : function(record, rowIndex, colIndex, options){
			...
			/*if(colIndex==7){
				var link1 = $('<a>挂起</a>').click(function(e) {
					comm.i_confirm(comm.lang('pointWelfare').mfsuspendGrant,function(){
		...
		del : function(record, rowIndex, colIndex, options){
			if(colIndex==7){
				var link1 = $('<a>拒绝受理</a>').click(function(e) {
					comm.i_confirm(comm.lang('pointWelfare').mfrefuseGrant,function(){
