define([
"text!scoremanageTpl/jfflff/ywshbtff.html",
...
'scoremanageLan'
],function(ywshbtffTpl,ffTpl,sq_dialogTpl,jfflff_gq_dialog,pointWelfare){
 
	var self = {
			showPage : function(){
				$('#busibox').html(_.template(ywshbtffTpl));	
				/** 查询事件 */
		...
		pageQuery:function(){
			...
			//查询参数
			var queryParam={
							"search_welfareType":"0",	//int	否	福利类型 0 意外伤害 1 免费医疗 2 他人身故
		...
		del : function(record, rowIndex, colIndex, options){
			if(colIndex==8){
				var link1 = $('<a>拒绝受理</a>').click(function(e) {
					comm.i_confirm(comm.lang('pointWelfare').ywrefuseGrant,function(){
