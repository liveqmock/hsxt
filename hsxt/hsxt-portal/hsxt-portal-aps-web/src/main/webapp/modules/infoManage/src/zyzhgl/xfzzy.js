define(['text!infoManageTpl/zyzhgl/xfzzy.html',
		'text!infoManageTpl/zyzhgl/xfzzy_sz.html',
		'text!infoManageTpl/zyzhgl/xfzzy_sz_qr.html',
        'infoManageDat/infoManage',
		'infoManageLan'
		/*'text!infoManageTpl/zyzhgl/xfzzy_ck.html'*/
	], function(xfzzyTpl, xfzzy_szTpl, xfzzy_sz_qrTpl, dataModoule/*, xfzzy_ckTpl*/){
	var self= {
		//self : null,
		showPage : function(){
			//self = this;
		 ...
		 */
		initForm : function(){
			$('#busibox').html(_.template(xfzzyTpl));
			comm.initSelect('#search_realnameStatus', comm.lang("infoManage").custRealNameAuthSatus, null, null, {name:'全部', value:''});
		...
		initData : function(){
			...
			var params = {
					...
					search_realnameStatus:comm.removeNull($("#search_realnameStatus").attr('optionValue')),
			...
			cacheUtil.findCacheContryAll(function(){
				...
				dataModoule.findResConsumerInfoList(params, self.detail);
			});
		},
		/**
		 * 查看动作
		...
		detail : function(record, rowIndex, colIndex, options){
			if(colIndex == 0){
				return $('<a>'+ record['hsResNo'] +'</a>').click(function(e) {
					$("#ent_list").hide();
					dataModoule.queryConsumerAllInfo({'custID':record.custId},function(res){
					require(['infoManageSrc/zyzhgl/sub_tab_p'], function(tab){
						$("#menuId").val("#xfzzy");
			...
			}else if(colIndex == 2){//国籍
				return comm.isEmpty(record['countryName']) ? '' : cacheUtil.getCountryNameByNo(record['countryName']);
			}else if(colIndex == 5){//消费者状态
				return comm.getNameByEnumId(record['realnameAuth'], comm.lang("infoManage").consumerRealNameAuthSatus);
			}else if(colIndex == 7){//互生卡状态
				return comm.getNameByEnumId(record['baseStatus'], comm.lang("infoManage").HSCardStatusEnum);
			}else if(colIndex == 8){//操作
		...
		chaKan : function(obj){
			dataModoule.queryConsumerAllInfo({'custID':obj.custId},function(res){
...
//				});
		...
		validDate:function(){
			var realnameStatus = $("#search_realnameStatus").attr('optionValue');
		...
		set : function(record,obj){
			$('#sz_dlg').html(_.template(xfzzy_szTpl,obj));
			...
			$( "#sz_dlg" ).dialog({
				...
				buttons:{ 
					...
					//	$('#sz_qr_dlg').html(xfzzy_sz_qrTpl);
						$('#sz_qr_dlg').html(_.template(xfzzy_sz_qrTpl,obj));
						...
						$("#personName").text(record.perName);
