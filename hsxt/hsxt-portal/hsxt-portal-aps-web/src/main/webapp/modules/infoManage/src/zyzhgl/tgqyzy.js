define(['text!infoManageTpl/zyzhgl/tgqyzy.html',
		'text!infoManageTpl/zyzhgl/tgqyzy_sz.html',
		'text!infoManageTpl/zyzhgl/tgqyzy_sz_qr.html',
        'infoManageDat/infoManage',
		'infoManageLan'
		], function(tgqyzyTpl, tgqyzy_szTpl, tgqyzy_sz_qrTpl, dataModoule){
	var self= {
		//self : null,
		showPage : function(){
			//self = this;
		...
		initForm : function(){
			//var self = this;
			$('#busibox').html(_.template(tgqyzyTpl));
		...
		detail : function(record, rowIndex, colIndex, options){
			if(colIndex == 0){//企业互生号
				return $('<a>'+ record['hsResNo'] +'</a>').click(function(e) {
					...
					require(['infoManageSrc/zyzhgl/sub_tab'], function(tab){
						...
						tab.showPage('tgqy', record, record.custId);
					});
			...
			}else if(colIndex == 7){//参与积分状态
				return comm.getNameByEnumId(record['participationScore'], comm.lang("infoManage").pointStatus);
			}else if(colIndex == 9){//操作
		...
		set : function(record,obj){
			...
			$('#sz_dlg').html(_.template(tgqyzy_szTpl,obj));
			...
			$( "#sz_dlg" ).dialog({
				...
				buttons:{ 
					"确定":function(){
						...
						$('#sz_qr_dlg').html(_.template(tgqyzy_sz_qrTpl,obj));
					//	$('#sz_qr_dlg').html(tgqyzy_sz_qrTpl);
		 ...
		 * 表单校验
		 */
		tgqyzy_sz_form_validateData : function(){
			 var validate = $("#tgqyzy_sz_form").validate({
				 rules : {
					 jfzhsb_info : {
						required : true
					},
				...
				messages : {
					jfzhsb_info : {
						required : comm.lang("infoManage").describNotNull
					},
					jftz_info : {
						required : comm.lang("infoManage").describNotNull
					},
