define(['text!infoManageTpl/zyzhgl/cyqyzy.html',
		'text!infoManageTpl/zyzhgl/cyqyzy_sz.html',
		'text!infoManageTpl/zyzhgl/cyqyzy_sz_qr.html',
        'infoManageDat/infoManage',
		'infoManageLan'
		], function(cyqyzyTpl, cyqyzy_szTpl, cyqyzy_sz_qrTpl, dataModoule){
	var self= {
	//	self : null,
		showPage : function(){
			//self = this;
		 ...
		 */
		initForm : function(){
			$('#busibox').html(_.template(cyqyzyTpl));
			//时间控件		    
			...
			if(colIndex == 0){//企业互生号
				return $('<a>'+ record['hsResNo'] +'</a>').click(function(e) {
					...
					require(['infoManageSrc/zyzhgl/sub_tab'], function(tab){
						...
						tab.showPage('cyqy', record, record.custId);
					});
				...
				return comm.getNameByEnumId(record['baseStatus'], comm.lang("infoManage").pointStatus);
			}else if(colIndex == 8){//操作
				return $('<a>设置</a>').click(function(e) {
					var entaBaseStatus = record.baseStatus;
					if(5 == entaBaseStatus){
						comm.error_alert("已注销的企业状态禁止做【设置】的业务操作");
						return;
					}
		...
		set : function(record,obj){
			$('#sz_dlg').html(_.template(cyqyzy_szTpl,obj));
			...
			$( "#sz_dlg" ).dialog({
				...
				/*		if(self.changVal(jftzOld,"jftz","jftz_info","#jftz_text")){
							return false;
						}*/
						...
						if('0' == jfzhsb_status && '0' == dhhsb_status && '0' == hsbzhb_status && '0' == hbzyh_status){
							comm.i_alert("业务未有任何变化");
							return ;
						}
						$( "#sz_dlg" ).dialog( "destroy" );
					//	$('#sz_qr_dlg').html(cyqyzy_sz_qrTpl);
						$('#sz_qr_dlg').html(_.template(cyqyzy_sz_qrTpl,obj));
						/*弹出框*/
						$( "#sz_qr_dlg" ).dialog({
							title:"业务操作许可设置确认",
							width:"550",
