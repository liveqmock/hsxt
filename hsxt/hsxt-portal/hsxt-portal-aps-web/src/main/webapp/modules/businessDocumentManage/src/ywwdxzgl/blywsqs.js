define(['text!businessDocumentManageTpl/ywwdxzgl/blywsqs.html',
		'text!businessDocumentManageTpl/ywwdxzgl/edit.html',
		'text!businessDocumentManageTpl/ywwdxzgl/sub.html',
        'businessDocumentManageDat/businessDoc',
		'businessDocumentManageLan'
	], function(blywsqsTpl, editTpl, subTpl, dataModoule){
	return {
		blywsqs_self : null,
		showPage : function(){
			blywsqs_self = this;
		 ...
		 */
		initForm : function(){
			$('#busibox').html(_.template(blywsqsTpl));
			comm.initSelect('#search_docStatus', comm.lang("businessDocumentManage").statusEnum, null, null, {name:'全部', value:''});
			$("#queryBtn").click(function(){
				blywsqs_self.initData();
			});
			$("#xzmb_btn").click(function(){
				blywsqs_self.editData(null, null, "新增办理业务申请书");
			});
			blywsqs_self.initData();
		},
		editData : function(record, docCode, title){
			$('#sltpgl-edit-dialog > p').html(_.template(editTpl, record));
			blywsqs_self.getDocList(comm.removeNull(docCode));
			$('#gjcx_sltpmc').next('ol').css('margin-top',28);
			...
			$('#sltpgl-edit-dialog').dialog({
				...
				buttons: {
					"确定":function(){
						blywsqs_self.saveDoc("#sltpgl-edit-dialog");
					},
					"取消":function(){
						$(this).dialog("destroy");
					}
		...
		getDocList:function(defaultVal){
			comm.initSelect('#docCode', comm.lang("common").sampleBusEnum, 180, comm.removeNull(defaultVal), null, 100);
		},
		/**
		 *初始化数据
		 */
		initData : function(){
			...
			dataModoule.findBusinessBizDocList(params, this.detail_1, this.detail_2, this.detail_3);
		},
		/**
		 *自定义函数1
		 */
		detail_1 : function(record, rowIndex, colIndex, options){
			...
			if(record.status == 2){
				return $('<a>停用</a>').click(function(e) {
					$('#sltpgl-edit-dialog > p').html(_.template(subTpl, record));
					$('#sltpgl-edit-dialog').dialog({
						title : "提示",
						open: function (event, ui) {
							$(".ui-dialog-titlebar-close", $(this).parent()).show();
						...
						buttons: {
							"确定":function(){
								if(!blywsqs_self.validateData_2().form()){
									return;
								}
								//验证双签
		...
		detail_2 : function(record, rowIndex, colIndex, options){
			...
			if(record.status != 2){
				return $('<a>编辑</a>').click(function(e) {
					blywsqs_self.editData(record, record.docCode, "修改办理业务申请书");
				}.bind(this));
			}
		},
		/**
		...
		saveDoc : function(dialogId){
			if(!blywsqs_self.validateData().form()){
				return;
			}
			var delFileIds = [];
			if($("#fileId").val() != ""){
				...
				comm.uploadFile(null, ['fileId'], function(data){
					...
					blywsqs_self.saveBusinessDoc(dialogId);
				}, function(err){
		...
		saveBusinessDoc : function(dialogId){
			...
			params.docType = 2;//文件类型
			dataModoule.saveBusDoc(params, function(res){
