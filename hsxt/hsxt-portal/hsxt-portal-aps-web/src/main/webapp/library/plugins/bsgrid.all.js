    ...
    $.fn.bsgrid = {
        ...
        init: function (gridId, settings) {
            ...
            var gridObj = {
				...
				/* whc 
				 * add functions delRow ,editRow 
				 *
				 */
				delRow : function(row,options){
					alert(row)
					//return $.fn.bsgrid.delRow(row, options);
				},
				
				editRow : function(row,options){
					return $.fn.bsgrid.editRow(row, options);
				},
                                ...
                                if (render != '') {
									...
									switch (render){
										...
										case 'operate':
											...
											var add_html ,del_html,edit_html;
											
											var operate = options.settings["operate"];
											
											if (operate.add){
												render_method = eval(operate.add) ;	
												add_html = render_method(record, i, j, options);
                                                tdObj.append('<span>&nbsp;&nbsp;</span>');
												tdObj.append(add_html);
											} 
											if (operate.del){
												render_method = eval(operate.del) ;	
												del_html = render_method(record, i, j, options);
												tdObj.append('<span>&nbsp;&nbsp;</span>');
												tdObj.append(del_html);
											} 
											if (operate.edit){
												render_method = eval(operate.edit) ;
												edit_html = render_method(record, i, j, options);	
												tdObj.append('<span>&nbsp;&nbsp;</span>');
												tdObj.append(edit_html);
											}
											break;
										case 'renderImg' :
											var renderImg = options.settings["renderImg"];
												render_method = eval(render) ;
											var img_html = render_method(record, i, j, options);
											tdObj.append(img_html);											
											break;
										default : 
...
/***中文语言***/
(function(a){a.bsgridLanguage={isFirstPage:"已经是第一页！",isLastPage:"已经是最后一页！",needInteger:"请输入数字！",needRange:function(c,b){return"请输入一个在"+c+"到"+b+"之间的数字！"},errorForRequestData:"当前账号可能已在异地登录，您已被提出！",errorForSendOrRequestData:"发送或请求数据失败！",noPagingation:function(b){return'共:&nbsp;<span id="'+b+'"></span>'},pagingToolbar:{pageSizeDisplay:function(d,c){var b="";if(!c){b+="每页显示:"}return b+'&nbsp;<select id="'+d+'"></select>'},currentDisplayRows:function(c,b,e){var d="";if(!e){d+="当前显示:"}return d+'&nbsp;<span id="'+c+'"></span>&nbsp;-&nbsp;<span id="'+b+'"></span>'},totalRows:function(b){return'共:&nbsp;<span id="'+b+'"></span>'},currentDisplayPageAndTotalPages:function(b,c){return'<div><span id="'+b+'"></span>&nbsp;/&nbsp;<span id="'+c+'"></span></div>'},firstPage:"首&nbsp;页",prevPage:"上一页",nextPage:"下一页",lastPage:"末&nbsp;页",gotoPage:"跳&nbsp;转",refreshPage:"刷&nbsp;新"},loadingDataMessage:"正在加载数据，请稍候......",noDataToDisplay:"没有数据可以用于显示。"}})(jQuery);
