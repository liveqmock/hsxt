define(['text!systemManageTpl/czygl/jbhsk.html',
	...
	return jbhsk = {
		...
		showPage : function(record){
			...
			$('#busibox').html(_.template(hskbdTpl,record)) ;			 
		  	...
		  	$('#btn_jbhsk_cx').triggerWith('#xtyhgl_czygl');
			
			$('#btn_jbhsk_tj').click(function(){
				
				if(!jbhsk.validateData()){
					return;
				}
				...
				//获取用户登录信息
			 	...
			 	comm.requestFun("getToken", null, function(res){
			 		...
			 		systemManage.unBindCard(postData,function(res1){
			 			
			 			$("#userNameTd").text(record.userName);
			 			$("#realNameTd").text(record.realName);
			 			$("#hskhTd").text(record.operResNo);
			 			$('#hskjb_dialog').dialog({
			 				width:480,
			 				buttons:{
			 					'确定':function(){
