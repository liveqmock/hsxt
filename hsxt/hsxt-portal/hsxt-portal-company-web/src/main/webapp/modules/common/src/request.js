define(['GY','commSrc/commUrl', 'commSrc/comm'], function(GY,url) {
    ...
    function formatOptions(options){
        ...
        options.error = function(response,type) {
			...
			if(!response.isAbort){
				if(!response.noerror){		
					/* 2016-03-15  whc  弹提示框需取消其它提示框  begin*/
					$('.loading_div,.lockscreen').hide();		 
					$('.ui-dialog').remove();
					/* end */
//					comm.i_alert("网络连接超时,请检查您的网络设置或重试？");			
					return;	 
				}
			}
        }
        ...
        var success = function(response) {
			...
			*/
			...
			if(response && response.retCode){ 
					...
					if(errCode && _.contains(comm.retCode,errCode) ){
						comm.killRequest();							
						//特殊代码处理						
						...
						$('.bsgrid').hide();//add by zhucy 2016-03-29 隐藏grid 表格的loadding
						...
						switch(errCode){
							...
							case comm.retCode[8]:{
								var errMsg = '账号在其他地方登陆，已被踢出!请重新登录！';
								dialogEven(dialog ,errCode,errMsg)	;																
								break;								
							}
							case comm.retCode[9]:{
								var errMsg = 'session过期，请重新登录!';
								dialogEven(dialog ,errCode,errMsg)	;
								break;								
							}
							case comm.retCode[10]:{
								var errMsg = '系统未获取到网上商城信息或网上商城未开通,请先开通网上商城！';
								/*comm.i_confirm(errMsg,function(){
									location.href = comm.domainList.homePage;
								},550);*/
								
								/*提示信息框布局优化 modify by kuangrb 2016-04-18*/
								comm.i_alert(errMsg,600);
								/*end*/
								break;
							}
							case comm.retCode[11]:{
							    var errMsg = '网上商城申请启用尚未审核通过,请联系服务公司人员进行咨询！';
								comm.i_alert(errMsg,550);
								break;	
							} 
							case comm.retCode[12]:{
							    var errMsg = '无权限访问,请联系管理员！';
								comm.i_alert(errMsg,550);
								break;
							} 
							case comm.retCode[13]:
							case comm.retCode[14]:
							/*	var errMsg = '当前登录用户已经给踢出或session过期,请重新登录!';
								comm.i_confirm(errMsg,function(){
									location.href = comm.domainList.homePage;
								},550);*/
								
								$("#warn_content").html('当前登录用户已经给踢出或session过期,请重新登录!');
				    			$("#alert_warn").dialog({
				    				title : "提示信息",
				    				width : "550",
				    				modal : true,
	...
	function errCodeEven(evenObj,dialog){
		if (evenObj && evenObj.length>0 ){
			for(var j = 0 ;j < evenObj.length ;j++){				
				...
				dialog[j].retCode = evenObj[j].retCode ;
