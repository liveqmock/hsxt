		...
		bindLiClick : function(liId, msg){
			...
			$(liId).click(function(){
				...
				if(msg.msgType == 1001 || msg.msgType == 1002 || msg.msgType == 1003 || msg.msgType == 1004 || msg.msgType == 1005){
					...
					if(msg.msgType == 2531 || msg.msgType == 2532){
						var hrefId = liId.replace("#", "")+"href";
						$("#pushContent").html(msg.content+"<a href=javascript:void(0); id="+hrefId+">点击查询</a>");
						$("#"+hrefId).unbind('click').bind('click', function(){
							try{
								$('#xtxx_box').addClass('none');
								$("#msg-pv-tab").empty();
								pushService.systemOpenList = {};
								//模拟点击动作
								if(!$(".icon_nav_11").parent().hasClass("navMenu_hover")){
									$(".icon_nav_11").click();
								}
								if(!$("#020100000000_subNav_020102000000").hasClass("active")){
									setTimeout(function(){
										$("#020100000000_subNav_020102000000").click();
									}, 500);
								}
								setTimeout(function(){
									$("#hsbzh_mxcx").click();
