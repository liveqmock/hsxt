define(["hsec_shopCateFoodDat/shopCate",
	...
	var queryParam = {"eachPageSize":10};
	var curPage = 1;
	var https = "";
	var industry = "";
	var responseDetail = {};
	...
	var shopCate = {
		...
		loadBsGridData : function(param){
			var gridObj;
			$(function(){
				gridObj = $.fn.bsgrid.init('cateoryGridMain',{
					url:{url:comm.UrlList["queryShopCategoryGrid"],domain:"sportal"},//comm.domainList["sportal"]+comm.UrlList["queryShopCategoryGrid"],
					pageSize:10,
					otherParames:param,
					...
					operate:{
						add : function(record, rowIndex, colIndex,options){
							...
							switch (colIndex) {
								...
								case 1 :
									if(picUrl != ''){
										sTpl = '<img  width="40" height="40" class="bigImg '+id+'"/>';//'<img src="'+https+picUrl+'" width="40">';
									}else{
										sTpl = '';
									}
									break;
						...
						if(param.industry=='1'){
							...
							$(".list-up-arrow,.list-down-arrow").click(function(){
								if(gridObj.getSelectedRowIndex()==-1){
									tablePoint.tablePoint("请先选中行")
								}else{
								  index = gridObj.getSelectedRowIndex();
								  $this = $(this);
								  ...
								  if($this.hasClass("list-up-arrow")){
									 if(index==0){
										 param.replaceId="";
									 	 ...
									 	 --index;
									 	 param.replaceId=gridObj.getRecord(index).id;
									 }
								  }else{
									 if(index==(gridObj.getRows().length-1)){
										 param.replaceId="";
									 }else{
									 	 ++index;
									 	 param.replaceId=gridObj.getRecord(index).id;
		...
		detailItemOprate : function(obj){
			...
			var queryRalationParam={};	//查询关联的商品
				...
				queryRalationParam["curPage"] = curPage;  	
		...
		addItemRealationGrid : function(obj){
			...
			var queryNotRalationParam={};	//查询关联的商品						
				...
				queryNotRalationParam["currentPageIndex"] = 1;
	  			...
	  			//查询
				$("#cgbox3 #searchItem_Listhead2").on("click",function(){
	  				var queryNotRalationParam={};	//查询关联的商品						
					queryNotRalationParam["shopCategoryId"]=obj;		
					queryNotRalationParam["status"]=3;		//查询已经上架的商品			
					queryNotRalationParam["curPage"] = curPage;
					queryNotRalationParam["name"]=$.trim($("#itemName_Listhead2").val());
		    	    queryNotRalationParam["brandName"]=$.trim($("#itemBrand_Listhead2").val());	
	  				var paramItems = {"jsonItemInfo" : JSON.stringify(queryNotRalationParam)}; 
	  				paramItems.industry= "1";	//用来区分查询哪个表餐饮或零售
	  				paramItems.searchStatus="0";	//查询未关联的商品 
		    	    shopCate.loadAddGridData(paramItems);  					  			
		...
		loadAddGridData : function(queryParam){
			var gridDetailObj = $.fn.bsgrid.init('gridItemAdd',{
					...
					add : function(record, rowIndex, colIndex, options){
						...
						switch (colIndex) {
							case 0 :
								...
								break;
							case 4 :
								sTpl = '<a class="addItemRelation" data-itemId="'+record.strId+'">设置分类商品</a>';
								break;
							default :
								break;
						}
				...
				complete : function(e,o){
					 tablePoint.checkBoxAll($("#all_item,#all_item2"),$("input[name='itemDataCheckBox']"));
					 $(".addItemRelation").click(function(){
					 	var $this = $(this);
					 	comm.i_confirm("确定新增关联?", function() {
						   var addItemRelationParam={};	
						　　　　addItemRelationParam.itemIds= $this.attr("data-itemId")+",";	//商品id多个商品，号分开
						   　　　addItemRelationParam.salerCategoryId=shopCategoryId;			//商城分类id	
						   　　　addItemRelationParam.industry = 1;
						   	   shopCate.addSalerShopCategoryFn(addItemRelationParam);
						  });
					 });
				}
			});
			...
			$(".batchAddItemRelation").click(function(){
				 var addClick=$(this);
		    	   comm.i_confirm("确定新增关联?", function() {
				   var addItemRelationParam={};	
		...
		addSalerShopCategoryFn : function(param){
			tablePoint.bindJiazai($(".operationsInner"),true);	 
	    	ajaxRequest.addSalerShopCategory(param, function(response) {
	    		if (response.retCode == "200") {
	    				//刷新商品菜单
		    			tablePoint.bindJiazai($(".operationsInner"),false);	 
						tablePoint.tablePoint("操作成功！",function(){
						$("#cgbox3 #searchItem_Listhead2").click();  
					});											 																											
				}else{
					tablePoint.tablePoint("操作失败!");
				}
		...
		loadDetailGridData : function (queryParam){
			var gridDetailObj = $.fn.bsgrid.init('gridItemDetail',{
					...
					add : function(record, rowIndex, colIndex, options){
						...
						switch (colIndex) {
							case 0 :
								...
								break;
							case 4 :
								sTpl = '<a class="cancelItemRelation" data-itemId="'+record.strId+'">删除</a>';
								break;
							default :
								break;
						}
				...
				complete : function(e,o){
					 tablePoint.checkBoxAll($("#all_item,#all_item2"),$("input[name='itemDataCheckBox']"));
					//shopCate.cancelItemRealation($(this).attr("data-itemId"),$this);
					 //单个删除
					$(".cancelItemRelation").unbind().on("click",function(){
						var $this = $(this);
						comm.i_confirm("确定取消关联?",function(){
							var cancelItemRelationParam={};
							cancelItemRelationParam.itemIds=$this.attr("data-itemId")+",";
							cancelItemRelationParam.salerCategoryId=shopCategoryId;
							cancelItemRelationParam.industry = 1;
							shopCate.cancelPublicFn(cancelItemRelationParam);
						});
					});
				}
			});
			...
			//批量取消
			$(".batchCancelItemRelation").click(function(){
				var cancelClick=$(this);
				   ...
				   if($(cancelClick).hasClass("batchCancelItemRelation")){
			    	...
			    	cancelItemRelationParam.salerCategoryId=shopCategoryId;	
			    	cancelItemRelationParam.industry = 1;//商城分类id	
			    	shopCate.cancelPublicFn(cancelItemRelationParam);
			   })
			});
		},
		cancelPublicFn : function(param){
			tablePoint.bindJiazai($(".operationsInner"),true);	 
	    	ajaxRequest.delSalerShopCategory(param, function(response) {
	    		if (response.retCode == "200") {
	    				//刷新商品菜单
		    			tablePoint.bindJiazai($(".operationsInner"),false);	 
						tablePoint.tablePoint("操作成功！",function(){
						$("#cgbox2 #searchItem_Listhead").click();
					});											 																											
				}else{
					tablePoint.bindJiazai($(".operationsInner"),false);
					tablePoint.tablePoint("操作失败!");
			 ...
			 //新增零售分类
			  ...
			  $( "#dlg2" ).dialog({
					...
					ajaxRequest.getAddBaseData({},function(response){
						...
						var html = _.template(itemCateDialogTpl,response);
					  	...
					  	if(loadData!=null){
					  		 ...
					  		 if(loadData.parentId != 0){
					  			$("#parentSelect option:first").attr("disabled", 'disabled');
					  		 }
				...
				'保存': function() {
						 ...
						 //验证排序字段							 
						 if(comm.isEmpty(faParam["sortOrder"]) || (isNaN(faParam["sortOrder"])) || Number(faParam["sortOrder"]) > 255){
							 tablePoint.tablePoint("排序字段不能为空,且必须为数字,不能大于255!");
							 return false;
						 }
						 else{//验证字段为正整数 add by kuangrb 2016-05-27
							 var re = /^[1-9]+[0-9]*]*$/;
							 if(!re.test(faParam["sortOrder"])){
								 tablePoint.tablePoint("排序字段必须为正整数!");
								 return false;
							 } 
						 }
  				...
  				ajaxRequest.getRelationItem(paramItems, function(response) {
					...
					$("#cgbox2 #searchItem_Listhead").unbind().on('click',function(){	   
		    	    	...
		    	    	queryRalationParam["brandName"]=brandName;		  			  			
		  				curPage=curPage;		  				
		  				queryRalationParam["eachPageSize"] = 10;						
		  				$($this).click();  
					 });
					 tablePoint.checkBoxAll($("#all_item,#all_item2"),$("input[name='itemDataCheckBox']"));
					 //取消关联
...
//			$("#lsfllb tbody tr").hover(function(){
			$("#cateoryGridMain tbody tr").hover(function(){
						...
						var editData={
							...
							if (response.retCode == "200") {
								tablePoint.tablePoint("操作成功！",function(){									
									...
									//shopCate.loadBsGridData({"industry":"1"});
									//$("#foodCategory").click();
									$('#retailAssortment').trigger('click');
								});																		
							}else if(response.retCode == "206"){
								tablePoint.tablePoint("商品分类信息不符合规范，包含违禁字:<br>"+response.data+"<br>请修改之后再添加！")
							}else if(response.retCode == "305"){
					...
					//修改餐饮分类
					  ...
					  $( "#dlg3" ).dialog({
	  						...
	  						open : function(){
	  							...
	  							if($this.attr("objPicUrl") != ""){//修改Bug：24127
									...
									if (response.retCode == "200") {
										tablePoint.tablePoint("删除成功！",function(){
											...
											if(industry=='1'){
												...
												$('#foodCategory').trigger('click');	
											}
											//$('#retailAssortment,#foodCategory').trigger('click');
										});	
									}else{
										tablePoint.tablePoint(response.data);
									}
			...
			$(".cpxx").unbind().on('click',function(){
				...
				ajaxRequest.cateAndItemCollection(qureyRealationFood,function(response){
					if(response.data.result.length == 0){
						tablePoint.tablePoint3("该自定义分类还没有菜品关联,是否开始关联菜品?",function(){
