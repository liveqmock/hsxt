define([ "text!hsec_orderTpl/orderHead/orderHead.html",
         "text!hsec_orderTpl/orderData/orderData.html",
...
//						var opts = new Array();
//						opts[0] = {name:"关联营业点",value:""};
//						$.each(response.data,function(key,d){
//							opts[key+1] = {name:'【'+d.addAll+'】',value:d.id}
//						});
//						$("#selRelatedShop").selectList({
//							options:opts,
//							width:115
//						});
//					}
...
//			},
			...
			queryParam : function(orderStatusStr, currentPageIndex) {
		    	...
		    	var nickName =$("#ConsumerName").val();		
		    	var startTime=$("#startTime").val();
		    	var endTime =$("#endTime").val();
		    	var shopIds =$("#selRelatedShop").val();
...
//				shop.queryData(shop.queryParam(orderStatusStr,"1"));
			...
			queryData:function(queryParam){
				var data = {};
				var data2 = {};
				var gridObj = $.fn.bsgrid.init('gridMain',{
						...
						additionalRenderPerColumn: function(record, rowIndex, colIndex, tdObj, trObj, options){
							...
							sTpl +='			<%if((obj.delayDeliverCount == null || obj.delayDeliverCount == "") || (obj.delayDeliverCount < obj.orderDelayCount2)){%>'
							...
							//订单详情信息html;
							sTpl_1 = '<td><div class="fl mr10"><a href="#"><img data-src="<%=JSON.parse(obj.picUrl)["68x68"]%>" height="40" class="orderImg"/></a></div>'
							sTpl_1 += '<div class="fl"><ul class="tl"><li class="w130" title="<%=obj.itemName%>" ><span style="word-wrap: break-word;"><%=obj.itemName%></span></li>'
                      		sTpl_1 += '<%var skus = obj.skus.split(",")%>'
			      			sTpl_1 += '<%$.each(skus,function(k,vl){%>'
			       			sTpl_1 += '	<li class="mt5"><%=vl%></li>'
			       			sTpl_1 += '	<%})%>'
                   			sTpl_1 += '  </ul>'
                   			sTpl_1 += ' </div></td>';
							//从单价到操作html;
							sTpl_2 = '<td><img src="resources/img/price.png" width="20" height="20" style="vertical-align:middle;"/><%=comm.formatMoneyNumber(obj.orderDetailList[0].price)%></td><td><%=obj.orderDetailList[0].quantity%></td><td><img src="resources/img/price.png" width="20" height="20" style="vertical-align:middle;"/><span class="red"><%=comm.formatMoneyNumber(obj.actuallyAmount)%></span><p>含运费（<%=comm.formatMoneyNumber(obj.postAge)%>）</p></td>'
							sTpl_2 += '<td><img src="resources/img/pv.png" width="25" height="14" style="vertical-align:middle;"/><%=comm.formatMoneyNumber(obj.totalPoints)%></td>'
							sTpl_2 += '<td><% if(obj.deliveryType==1){%>快递<%}else if(obj.deliveryType==2){%>营业点自提<%}else if(obj.deliveryType==3){%>送货上门<%}%></td>'
							sTpl_2 += '<td><% if(obj.payType==1){%>现金<%}else if(obj.payType==3){%>互生币<%}else if(obj.payType==4){%>网银<%}%></td>'
							...
							for(var i = len - 1; i >= 0; i--){
								...
								$tr.append(_.template(sTpl_1, obList[i]));//渲染订单详情信息
								if(i === 0) {
						...
						complete:function(options, XMLHttpRequest, textStatus){
							...
							if(data2.data.length<=0){
								$("#gridMain").find("tbody tr td").attr("colspan","9");
							}
							other = data2.orther;
							if(other == null){
								return;
							...
							if(data2.data.length > 0){
								...
								$("#gridMain").find("tbody tr td").attr("colspan","9");//合并单元格
							}
	    		    		//end start time;
							shop.bindHeadClick();
							//延迟收货
			    ...
			    orderDelete : function(obj){
			    	...
			    	yanchiTime:function(obj){
			  		  ...
			  		  $( "#dlg3" ).dialog({
							...
							modal: true,
							open : function(){
								$(this).html("<p class='table-del'>订单收货时间将延长,确定延时?</p>");//"+yanchiTime+"小时
							},
							buttons: {
								"确定": function() {
									 	$(this).dialog('destroy');
		    	  ...
		    	  sendItem:function(obj){
		    		    ...
		    		    ajaxRequest.querySalerShopDeliver(param,function(response){
								...
								$("#dlg2").dialog({
									title : "选择送货人",//display: block; width: 546px; min-height: 50px; max-height: none; height: 227px;
									width : 600,
									height:300,
									modal : true,
									close: function() { 
