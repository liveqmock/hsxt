// 添加
define([ "text!hsec_shopInfoTpl/shopInfoAdd/shopInfoAdd.html"
         ...
         ], function(shopInfoAddTpl,baiduMapTpl,ajaxRequest,shopInfoCheckJs,tablePoint) {
	var category;//主营分类
	var Location;//商圈
	var shop;
	var shop2Add;
	var shopInfoAdd = {
			init : function(shopMain,shopAdd){
				shop = shopMain;
				shop2Add = shopAdd;
				if(Location!=null){
					Location = '';
			...
			add : function(){
				...
				ajaxRequest.getAllMainPageCategory(null, function(response) {	
					tablePoint.bindJiazai(".operationsArea", false);	
					category = response.data.MainPageCategory;
					Location = response.data.Location;
					var html = _.template(shopInfoAddTpl, response.data);
					$('#stdbj').html(html);
					shopInfoAdd.bindClick();
					shopInfoAdd.bindTableTwoClick();
				})
			},
			bindClick : function(){
			...
			bindSubmit : function(){
				...
				ajaxRequest.saveSalerShop(param, function(response) {
					if(response.retCode == 200){
						tablePoint.tablePoint("添加营业点成功!",function(){
							//shop.bindData();
							$(".stdlb").click();
						});
					}else if(response.retCode=="304"){
						tablePoint.tablePoint("新增营业点数量已经超过上限数:"+response.data+",不能再新增！");
						...
						tablePoint.tablePoint("添加营业点失败!请稍后重试!");
					}
