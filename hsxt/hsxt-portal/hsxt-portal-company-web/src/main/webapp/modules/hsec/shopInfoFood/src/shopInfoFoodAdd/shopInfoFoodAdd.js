define(["hsec_tablePointSrc/tablePoint",
        "text!hsec_shopInfoFoodTpl/shopInfoFoodAdd/shopInfoFoodAdd.html",
		...
		function(tablePoint,shopInfoFoodAddTpl,baiduMapTpl,shopFoodAjax,shopInfoFoodCheckJs,shopInfoAddJs,tabTpl) {
			var Location;//商圈
			var shopMain = null;
			var shop = {
				bindFootInit : function(info) {
					shopMain = info;
					tablePoint.bindJiazai(".operationsArea", true);
					shopFoodAjax.findFoodInit(function(response) {
...
//						$('.operationsArea').html(html);
				...
				$("#qiTaStr").on("keyup keypress charcount change",function(){
					...
					$("#shopType").on("change",function(){
						...
						if(shopType == 1){
							shopInfoAddJs.init(shopMain,shop);
						}
					})
...
//					$('#imgUploadUl').on("mouseover mouseout","li",function(event){
//					 if(event.type == "mouseover"){
//					  //鼠标悬浮
//						 
//					 }else if(event.type == "mouseout"){
...
//					})
				...
				bindFuWuClick : function(){
					...
					//预约规则业务变更修改《modifly by zhanghh 2016-04-23
				...
				submit : function(){
					$("#submit").on("click",function(){
						...
						/**营业点详情*/
						shopFoodAjax.addFoodShop($.param(param),function(response) {
							if(response.retCode == 200){
								tablePoint.tablePoint("新增营业点成功!",function(){
									//shopMain.bindData();
									$(".stdlb").click();
								})
							}else if(response.retCode == 210){
								tablePoint.tablePoint("新增营业点名称已存在!请修改后重试!");
							} else{
								tablePoint.tablePoint("新增营业点失败!请稍后重试!");
							}
						})
					})
					//经营模式
					...
					function shopTypeList($this){
						...
						var trList = $($this).find("[type='checkbox']");
					...
					function shopTimeList(){		
						...
						for (var i = 1; i < 3; i++) {
							var paramTime = {};
							var timeSelect = $(".time"+i+" select").find("option:selected");
							var time = $(timeSelect).val();
							var timeName = $(timeSelect).eq(0).text();
							if(time != null && time != ""){
					...
					function supportServiceDesc(){
						...
						if($("#yuyue").is(":checked")){
							var param = {};
							param["baiFenBi"] = $.trim($("#yuyueBaiFenBi").val());//
