define([ "hsec_tablePointSrc/tablePoint",
	...
	function jifen2(obj, state) {
		...
		if(price == null || price == 0){
			...
			$(obj).on('keyup',function(){
				...
				var baifenbiNum = jinzhun(baifenbi);
				var priceNum = jinzhun(price);
				var jifenzhi = (((price * priceNum) * (baifenbi * baifenbiNum)) / (baifenbiNum * priceNum)) / 100;
				if(Number(baifenbi) > 5){
				}else{
					jifenzhi = jifenzhi + 0.01;
				}
				jifenzhi = (jifenzhi / 2).toFixed(2); 
				tbody.children("td:eq(" + (y - 1) + ")").find("input").val(jifenzhi);
				tbody.children("td:eq(" + (y + 1) + ")").find("input").val(jifenzhi * 2);
				tbody.children("td:eq(" + (y + 2) + ")").find("input").val(jifenzhi);
			})
			if(state == 0){
		...
		function jinzhun(thisNum){
			var oStr=thisNum.toString(); 
			if(oStr.indexOf(".")==-1){
				return 1;
			}else{
	...
	function skuInput(ajax,obj){
		step.Creat_Table();	
		 //商品信息销售属性设置(SKU)
			 $("#skuMain").off().on('click','.sale_set',function(){
//				var $popup=$this.children(".popup_set");
//				$popup.is(":visible")?$popup.hide():$popup.show()
				});
			// 积分百分比修改事件
			$("#skuMain").off().on('focus','.jifen2',function(){
				jifen2(this, 1);
			});
			// 积分值修改事件
			$("#skuMain").off().on('focus','.jifen1',function(){
				jifen1(this, 1);
			});
	...
	function ImgLoad(color,isColor) {
		$("#imgLoad").append("<li colorId='"+color[0]+"'><div class=\"upload_hd\" style='width:80px'><span>"+$.trim(color[1])+"</span></div>"
	...
	function imgSkuUpload(ajax,obj){
        ...
        var file = $(obj).val();  
        var htmlUpload = $(obj).html();//这里取值有问题。2016-02-04
...
var inputData  = new Array();
	var step = {
		InitSpuSku : function(ajax) {
			...
			$("#skuMain").find("ul li").on("click",".skuName",function(e){
...
//				step.Creat_Table();
//			})
				$("#skuMain").on("click",".editval-del i",function(){
			...
			$("#itemMain").on("click",".editval-add",function(){
