            ...
            callCenter.searchEntAllInfo(param, function (request) {
                if (request.retCode == 22000) {
                   ...
                   /* $("#ldtp_mc").val(data.authInfo.userName);
                    ...
                    $("#cyqy_gsdjxx tr:eq(3) td:odd:eq(0)").html(data.mainInfo.entCustType);//企业类型
                    $("#cyqy_gsdjxx tr:eq(3) td:odd:eq(1)").html(data.mainInfo.busiLicenseNo);//营业执照号
                    ...
                    $("#cyqy_gsdjxx tr:eq(5) td:odd:eq(0)").html(data.extendInfo.buildDate);//成立日期
                    $("#cyqy_gsdjxx tr:eq(5) td:odd:eq(1)").html(data.extendInfo.endDate);//营业期限
                    $("#cyqy_gsdjxx tr:eq(6) td:odd:eq(0)").html(data.extendInfo.businessScope);//经营范围
                    ...
                    $("#cyqy_lxxx tr:eq(3) td:odd:eq(0)").html(data.baseInfo.entName);//联系人授权委托书   //需要独立接口
                    ...
                    $("#ldtp_b_xq_xtdjxx tr:eq(1) td:odd:eq(1)").html(data.baseInfo.entNameEn);//企业英文名称
                    cacheUtil.syncGetRegionByCode(data.baseInfo.countryCode, data.baseInfo.provinceCode, data.baseInfo.cityCode, "", function (resdata) {
                    ...
                    $("#ldtp_b_xq_gsdjxx tr:eq(2) td:odd:eq(0)").html(data.mainInfo.entCustType);//企业类型
                    ...
                    $("#ldtp_b_xq_gsdjxx tr:eq(4) td:odd:eq(0)").html(data.extendInfo.buildDate);//成立日期
                    $("#ldtp_b_xq_gsdjxx tr:eq(4) td:odd:eq(1)").html(data.extendInfo.endDate);//营业期限
                    $("#ldtp_b_xq_gsdjxx tr:eq(5) td:odd:eq(0)").html(data.extendInfo.businessScope);//经营范围
                    //联系信息
                    ...
                    $("#ldtp_b_xq_lxxx tr:eq(2) td:odd:eq(1)").html(data.baseInfo.contactEmail); //企业安全邮箱
            ...
            callCenter.getAccountBalance(param, function (request) {
                if (request.retCode == 22000) {
                    ...
                    if(comm.isNotEmpty(balance)){
                        /********************弹屏界面*******************/
                        $("#ldtp_b_pointBalance").html(comm.isNotEmpty(balance[10110]) ? balance[10110].accBalance : 0);//积分账户余数
                        $("#ldtp_b_pointBalance_usable").html(comm.isNotEmpty(balance[10110]) ? (balance[10110].accBalance - 10) : 0);//可用积分数
                        $("#ldtp_b_cashBalance").html(comm.isNotEmpty(balance[30110]) ? balance[30110].accBalance : 0);//货币账户余额
                        $("#ldtp_b_hsb_ltb").html(comm.isNotEmpty(balance[20110]) ? balance[20110].accBalance : 0);//流通币
                        $("#ldtp_b_hsb_csjz").html(comm.isNotEmpty(balance[20130]) ? balance[20130].accBalance : 0);//慈善救助金
                        $("#ldtp_b_point_tax").html(comm.isNotEmpty(balance[10510]) ? balance[10510].accBalance : 0);//积分税收金额总数
                        $("#ldtp_b_hsb_tax").html(comm.isNotEmpty(balance[20610]) ? balance[20610].accBalance : 0);//互生币税收金额总数
                        $("#ldtp_b_cash_tax").html(comm.isNotEmpty(balance[30310]) ? balance[30310].accBalance : 0);//货币(人民币)税收金额总数


                        /******************详单账户tab******************/
                        /**积分账户**/
                        $("#ldtp_b_xq_zhxx #company_point_balance").html(comm.isNotEmpty(balance[10110]) ? balance[10110].accBalance : 0);   //积分账户余数
                        $("#ldtp_b_xq_zhxx #company_point_balance_usable").html(comm.isNotEmpty(balance[10110]) ? (balance[10110].accBalance - 10) : 0);//可用积分数
                        $("#ldtp_b_xq_zhxx #company_point_yestday").html(point_yestday);//昨日积分数
                        /** 投资账户 **/
                        $("#ldtp_b_xq_zhxx #company_invest_sum").html(comm.isNotEmpty(balance[10410]) ? balance[10410].accBalance : 0);//积分投资总数
                        $("#ldtp_b_xq_zhxx #company_invest_income").html(invest_income_sum);//本年度投资分红互生币
                        $("#ldtp_b_xq_zhxx #company_invest_income_ltb").html(invest_income_ltb);//本年度流通币
                        $("#ldtp_b_xq_zhxx #company_invest_income_csjz").html(invest_income_csjz);//本年度慈善救助
                        /** 货币账户 **/
                        $("#ldtp_b_xq_zhxx #company_currency_balance").html(comm.isNotEmpty(balance[30110]) ? balance[30110].accBalance : 0);//货币账户余额
                        /** 互生币账户 **/
                        $("#ldtp_b_xq_zhxx #company_hsb_ltb").html(comm.isNotEmpty(balance[20110]) ? balance[20110].accBalance : 0);//流通币
                        $("#ldtp_b_xq_zhxx #company_hsb_csjz").html(comm.isNotEmpty(balance[20130]) ? balance[20130].accBalance : 0);//慈善救助金
                        /** 城市税收对接账户 **/
                        $("#ldtp_b_xq_zhxx #company_point_tax").html(comm.isNotEmpty(balance[10510]) ? balance[10510].accBalance : 0).attr("style", "margin-right:20px");//积分税收金额总数
                        $("#ldtp_b_xq_zhxx #company_hsb_tax").html(comm.isNotEmpty(balance[20610]) ? balance[20610].accBalance : 0).attr("style", "margin-right:20px");//互生币税收金额总数
                        $("#ldtp_b_xq_zhxx #company_currency_tax").html(comm.isNotEmpty(balance[30310]) ? balance[30310].accBalance : 0).attr("style", "margin-right:20px");//货币(人民币)税收金额总数
                    }
                }
            });
        },
