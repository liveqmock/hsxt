    ...
    $("#serviceSubNavBt").children().children().click(function (e) {
        ...
        switch (target) {
            ...
            case "trunktransferuser" :
                dialingObj.JS_TrunkTransferUser($("#dial_input").val());       //转内
                break;
            case "trunkcalltrunk" :
                dialingObj.JS_TrunkCallTrunk($("#dial_input").val());        //转外
                break;
            case "setuserquiet" :
    ...
    $("a[href=#jqp-tabs_1-6]").click(function () {
        callCenter.queryBusinessPmInfo(param, function (request) {
           /* if (request.retCode == 22000 && comm.isNotEmpty(request.data)) {
                ...
                if (data.PV_TO_HSB != 0) {
                    $("#jqp-tabs_1-6 tr:eq(0) td:odd  span").removeClass().addClass("red").html("限制");
                }    //积分转互生币
                if (data.PV_INVEST != 0) {
                    $("#jqp-tabs_1-6 tr:eq(1) td:odd  span").removeClass().addClass("red").html("限制");
                }    //积分投资
                if (data.BUY_HSB != 0) {
                    $("#jqp-tabs_1-6 tr:eq(2) td:odd  span").removeClass().addClass("red").html("限制");
                }    //兑换互生币
                if (data.HSB_TO_CASH != 0) {
                    $("#jqp-tabs_1-6 tr:eq(3) td:odd  span").removeClass().addClass("red").html("限制");
                }    //互生币转货币
                if (data.CASH_TO_BANK != 0) {
                    $("#jqp-tabs_1-6 tr:eq(4) td:odd  span").removeClass().addClass("red").html("限制");
                }    //货币转银行
            }*/
...
//查询银行卡列表
    ...
    $("a[href=#jqp-tabs_2-6]").click(function(){
        callCenter.queryBusinessPmInfo(param,function(request){
            /*if(request.retCode == 22000 && comm.isNotEmpty(request.data)){
                var data=request.data;
                if(data.PV_TO_HSB!=0)   { $("#jqp-tabs_2-6 tr:eq(0) td:odd  span").removeClass().addClass("red").html("限制");}    //积分转互生币
                if(comm.getResNoType(param.resNo)=="B"){    //成员企业没有积分投资
                    $("#jqp-tabs_2-6 tr:eq(1)").addClass("none");
                }else{
                    $("#jqp-tabs_2-6 tr:eq(1)").removeClass();
                    if(data.PV_INVEST!=0)   { $("#jqp-tabs_2-6 tr:eq(1) td:odd  span").removeClass().addClass("red").html("限制");}    //积分投资
                }
                if(data.BUY_HSB!=0)     { $("#jqp-tabs_2-6 tr:eq(2) td:odd  span").removeClass().addClass("red").html("限制");}    //兑换互生币
                if(data.HSB_TO_CASH!=0) { $("#jqp-tabs_2-6 tr:eq(3) td:odd  span").removeClass().addClass("red").html("限制");}    //互生币转货币
                if(data.CASH_TO_BANK!=0){ $("#jqp-tabs_2-6 tr:eq(4) td:odd  span").removeClass().addClass("red").html("限制");}    //货币转银行
            }*/
        ...
        callCenter.getAccountBalance(param, function (request) {
            if (request.retCode == 22000) {
                ...
                if(comm.isNotEmpty(balance)){
                    ...
                    $("#company_point_yestday").html(comm.formatMoneyNumberAps(point_yestday));//昨日积分数
                    /** 投资账户 **/
...
//查询账户余额
        ...
        callCenter.getAccountBalance(param, function (request) {
            if (request.retCode == 22000) {
                ...
                if(comm.isNotEmpty(balance)){
                    ...
                    $("#jqp-tabs_3-7 #p_point_today").html(comm.formatMoneyNumberAps(point_today));//今日积分数
                    /** 货币账户 **/
            ...
            callCenter.getPersonAllInfo(param, function (request) {
                if (request.retCode == 22000) {
                    ...
                    if(comm.isNotEmpty(userName)){
                        ...
                        comm.alert("未使用互生卡，不支持查询");
                    }
                    $("#p_username").html(userName);//姓名
                    $("#p_username").attr('title',data.authInfo.userName);//姓名
                    $("#p_username1").html(data.authInfo.userName);//姓名
                    ...
                    //互生卡身份信息
                    $("#p_person_resNo").html(p_resNo);    //互生卡号
                    $("#p_person_cre_type").html(comm.langConfig.common.creTypes[data.authInfo.cerType]);    //证件类型
                    $("#p_person_name").html(data.authInfo.userName);     //姓名
                    cacheUtil.syncGetRegionByCode(data.authInfo.countryCode, "", "", "", function (resdata) {
                        $("#p_nationality").html(resdata);    //国籍
                    });
                    $("#p_cre_no").html(data.authInfo.cerNo);    //证件号码
                    $("#p_job").html(data.authInfo.job);    //职业
                    $("#p_gender").html(comm.langConfig.common.sexTypes[data.authInfo.sex]);    //性别
                    $("#p_cre_end").html(data.authInfo.validDate);    //证件有效期
                    $("#p_cre_address").html(data.authInfo.birthAddress);    //户籍地址
                    $("#p_cre_gov").html(data.authInfo.issuingOrg);    //发证机关
                    $("#p_cre_gov").html(data.authInfo.issuingOrg);    //证件图片

                    var cerPica=comm.isNotEmpty(data.authInfo.cerPica)?'<a class="blue ml10 viewCardStyleImg" id="p_cre_img1" data-imgsrc="'+comm.domainList.fsServerUrl+data.authInfo.cerPica+'">查看正面照</a>':'';
                    var cerPicb=comm.isNotEmpty(data.authInfo.cerPicb)?'<a class="blue ml10 viewCardStyleImg" id="p_cre_img2" data-imgsrc="'+comm.domainList.fsServerUrl+data.authInfo.cerPicb+'">查看反面照</a>':'';
                    var cerPich=comm.isNotEmpty(data.authInfo.cerPich)?'<a class="blue ml10 viewCardStyleImg" id="p_cre_img3" data-imgsrc="'+comm.domainList.fsServerUrl+data.authInfo.cerPich+'">查看手持证件大头照</a>':'';
                    $("#p_cre_img a").remove();
                    $("#p_cre_img").append(cerPica).append(cerPicb).append(cerPich);
                    $('#p_cre_img .viewCardStyleImg').click(function(){
                        $('#viewConfirmCardStyle > p').html('<img src="'+$(this).attr('data-imgsrc')+'"/>');
                        $('#viewConfirmCardStyle').dialog({width:'auto',height:'auto',title:"证件预览" ,closeIcon:true,buttons:{"关闭":function(){$(this).dialog( "destroy" );}}});
                    });
                    //手机
    ...
    $("#search_company").click(function () {
        ...
        if ((comm.getResNoType(company_resNo) == "T" || comm.getResNoType(company_resNo) == "B")&& (comm.getSysCookie('entResNo').substr(0,2)==company_resNo.substr(0,2))) {
            //查询企业联系信息
            ...
            callCenter.searchEntAllInfo(param, function (request) {
                if (request.retCode == 22000) {
                    ...
                    $("#company_entCustName").attr('title',data.extendInfo.entCustName);  //企业名称
                    //托管成员企业详情
                    ...
                    $("#company_reg_date_1").html(data.statusInfo.openDate);//企业注册日期
    ...
    $("#search_scs").click(function () {
        var scs_resNo = $("#intpu_search_scs").val();
        if ((comm.getResNoType(scs_resNo.toString()) == "S") && (comm.getSysCookie('entResNo').substr(0,2)==scs_resNo.substr(0,2))) {
            //查询企业联系信息
            var param = {resNo: scs_resNo};
            callCenter.searchEntAllInfo(param, function (request) {
                if (request.retCode == 22000) {
                    ...
                    $("#scs_entCustName").html(ent_name.length<=15?ent_name:ent_name.substring(0,14)+"...");//企业名称
                    ...
                    $("#fwgsCreateDate").html(data.statusInfo.openDate);//企业注册日期
            ...
            comm.alert("请输入正确的本管理公司所属服务公司互生号");
        }
    });
    //end
    return tpl;
