define(['text!coDeclarationTpl/csyw/cyqycsyw.html',
        'coDeclarationDat/csyw/cyqycsyw',
	'commDat/common',
        'coDeclarationLan'],function(cyqycsywTpl, dataModoule,commAjax){
	var mcs_cyqycsyw = {
		bgGrid : null,
		showPage: function () {
			$('#cx_content_list').empty().html(_.template(cyqycsywTpl));
			...
			$('#queryBtn').click(function () {
				if (!comm.queryDateVaild('cy_search_form').form()) {
					return;
				}
				mcs_cyqycsyw.query();
			});
		...
		query: function () {
			...
			mcs_cyqycsyw.bgGrid =  dataModoule.findDeclareTrialList(jsonParam, this.detail, this.orderOpt);
		},
		/**
		 * 查看动作
		 */
		detail: function (record, rowIndex, colIndex, options) {
			if (colIndex == 6) {
				...
				return $('<a>' + comm.lang("coDeclaration").refuseAccept + '</a>').click(function (e) {
					comm.i_confirm(comm.lang("coDeclaration").firstMRefuseAccept, function () {
		...
		orderOpt: function (record, rowIndex, colIndex, options) {
			if (colIndex == 8) {
				return comm.workflow_operate(comm.lang("coDeclaration").hangUp, comm.lang("coDeclaration").firstMHangUp, function(){
					require(["workoptSrc/gdgq"],function(gdgq){
						gdgq.guaQi(record.applyId,mcs_cyqycsyw.bgGrid);
					});
				});
			}
		}
