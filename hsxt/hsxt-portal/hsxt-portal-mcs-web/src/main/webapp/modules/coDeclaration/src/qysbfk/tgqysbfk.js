define(['text!coDeclarationTpl/qysbfk/tgqysbfk.html',
        'coDeclarationDat/qysbfk/tgqysbfk',
	'commDat/common',
        'coDeclarationLan'],function(tgqysbfkTpl, dataModoule,commAjax){
	var mcs_tgqysbfk = {
		bgGrid:null,
		showPage: function () {
			$('#cx_content_list').empty().html(_.template(tgqysbfkTpl));
		...
		query: function () {
			...
			mcs_tgqysbfk.bgGrid=dataModoule.findDeclareReviewCodeList(jsonParam, mcs_tgqysbfk.detail,mcs_tgqysbfk.orderOpt);
		},
		/**
		 * 查看动作
		 */
		detail: function (record, rowIndex, colIndex, options) {
			if (colIndex == 6) {
				...
				return $('<a>' + comm.lang("coDeclaration").refuseAccept + '</a>').click(function (e) {
					comm.i_confirm(comm.lang("coDeclaration").secTRefuseAccept, function () {
		...
		orderOpt: function (record, rowIndex, colIndex, options) {
			if (colIndex == 8) {
				return comm.workflow_operate(comm.lang("coDeclaration").hangUp, comm.lang("coDeclaration").secTHangUp, function(){
					require(["workoptSrc/gdgq"],function(gdgq){
						gdgq.guaQi(record.applyId,mcs_tgqysbfk.bgGrid);
					});
				});
			}
		}
