define(['text!coDeclarationTpl/csyw/ckxq/qygsdjxx.html',
		'text!coDeclarationTpl/csyw/ckxq/qygsdjxx_dialog.html',
		'coDeclarationDat/csyw/ckxq/qygsdjxx',
	...
	var self =   {	 	
		...
		initShowForm : function(data, isCallBack){
			comm.delCache('coDeclaration', 'csyw-qygsdjxx-info');
			comm.setCache('coDeclaration', 'csyw-qygsdjxx-info', data);
			if(data){
		...
		initChangeData : function(){
			...
			var odata = comm.getCache('coDeclaration', 'csyw-qygsdjxx-info');
			var ndata = comm.cloneJSON(odata);
			var trs = "";
			var chg = {};
			for(var key = 0;key < ckArray.length; key++){
				if(comm.isNotEmpty(ckArray[key].value)){
