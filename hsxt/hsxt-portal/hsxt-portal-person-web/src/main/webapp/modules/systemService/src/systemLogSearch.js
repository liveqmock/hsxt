define(["text!systemServiceTpl/systemLogSearch.html",
        'systemServiceLan'], function (tpl) {
	var systemLogSearch = {
		_dataModule : null,
		...
		detail : function(record, rowIndex, colIndex, options){
			...
			else if(colIndex == 3)
			{
				if(methodName == "findTruenameregList"){//实名注册
					var bsResult = record['bsResult'];
					if(null != bsResult && '' != bsResult){
						return '成功';
					}
				//	return comm.getNameByEnumId(record['bsResult'], comm.lang("systemService").regStatus);
					...
					return comm.lang("systemService").bsResult[0];
				}
			}
			else if(colIndex == 4)
			{
