define(["text!hsbAccountTpl/detailSearch.html",
	...
	var detailSearch =  {
		show : function(){
			...
			//加载互生币类型
			comm.initSelectOption($("#shtc_hubtype"),comm.lang("hsbAccount").hsbAccTypeEnum);
			//加载业务类别
			comm.initSelectOption($("#shtc_subject"),comm.lang("hsbAccount").businessTypeEnum);
			//立即搜索单击事件
			$("#shtc_searchBtn").click(function () {
				var valid = comm.queryDateVaild("toCashAcctDetailSearch_form");
				if (!valid.form()) {
					return;
				}
				//分页查询
		...
		pageQuery:function(){
			var jsonData = {
					"search_businessType":comm.navNull($("#shtc_subject").val()),
					"search_accType": comm.navNull($("#shtc_hubtype").val()),
			...
			dataModule.detailedPage(jsonData,function(record, rowIndex, colIndex, options){
				...
				//渲染列:业务类型
				if(colIndex == 2){
					return comm.lang("hsbAccount").businessTypeEnum[record["businessType"]];
				} 
				//渲染列：发生金额
				if(colIndex==3){
		...
		getBusiType : function(transSys,transType,businessType){
			var transTypeName= comm.lang("hsbAccount").transTypeEnum[transType];
