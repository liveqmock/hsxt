define(["text!hsbAccountTpl/hsbBuy.html", 
		"text!hsbAccountTpl/payment.html"
	...
	var hsbBuy = {
		show : function(dataModule){
			...
			dataModule.initTransferHsb(jsonParam,function (response) {
				if (response) {
					...
					//加载数据
					$("#hb_currency").val(response.data.currencyCode);			//当前平台币种
					$("#currencyNameCn").text(response.data.currencyNameCn);			//当前平台币种名称
					$("#hb_rate").text(response.data.exchangeRate);			//当前兑换率
					$("#regMinAount_buyHsb").text(comm.formatMoneyNumber(response.data.regBuyHsbMin)); 			// 实名认证兑换互生最小值
					$('#regMinAount_buyHsbVal').val(response.data.regBuyHsbMin);
					
					$("#regMaxAount_buyHsb").text(comm.formatMoneyNumber(response.data.regBuyHsbMax)); 			// 实名认证兑换互生最大值
					$('#regMaxAount_buyHsbVal').val(response.data.regBuyHsbMax);
					
					$("#unRegMinAount_buyHsb").text(comm.formatMoneyNumber(response.data.noRegBuyHsbMin)); 	// 未实名认证兑换互生最小值
					$('#unRegMinAount_buyHsbVal').val(response.data.noRegBuyHsbMin);
					
					$("#unRegMaxAount_buyHsb").text(comm.formatMoneyNumber(response.data.noRegBuyHsbMax)); 		// 未实名认证兑换互生最大值
					...
					$("#hb_hid_isRegistration").val(response.data.authStatus); // 实名或者未实名标识
				}
			});
			...
			$('#hb_QrBtn').click(function(){
				...
				if("1" == restrictValue){
					...
					var msg = "兑换互生币业务暂时不能受理！<br />原因：" + restrictRemark;
					comm.warn_alert(msg);
					return;
				}
				//转出积分数数据验证
		...
		getMinAmount : function(){
			var arry = {};
			var isRegFlag = $("#hb_hid_isRegistration").val();
			if(isRegFlag == '1'){
