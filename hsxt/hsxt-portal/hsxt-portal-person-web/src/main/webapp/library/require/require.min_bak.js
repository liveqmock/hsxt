...
(function (aa) {
			...
			*/
			...
			'GY' : {
				deps : ['jquery', 'jqueryui', 'jquerycookie', 'jquerytimepicker', 'bsgrid',
					'jqueryjscrollpane', 'jquerymousewheel', 'jqueryselectbox',
					'jqueryvalidate', 'additionalmethods', 'jquerycustomer', 'jqueryuploadPreview', 'jqzoom',
					'jquerycutimage', 'jqueryajaxfileupload', 'underscore', 'commSrc/comm', 'commSrc/base', "localStorage", 
					//'jquerydataTables', "xheditor_cn"
					],
				exports : 'GY'
			}
		},
	...
	function ha(b) {
		...
		function t(a, f, b) {
			var e = a.id,
			n = m(p, e);
			if (s(q, e) && (!n || n.defineEmitComplete))
				"defined" === f && b(q[e]);
			else
				r(a).on(f, b)
		}
			...
			if (!W) {
				...
				G(V, function (b) {
					...
					if (b.enabled && (a.isDefine || g.push(b), !b.error))
						if (!b.inited && n)
							z(f) ? h = e = !0 : (c.push(f), d(f));
						else if (!b.inited && (b.fetched && a.isDefine) && (h = !0, !a.prefix))
							return j = !1
				});
				if (n && c.length)
					return b = B("timeout", "Load timeout for modules: " + c, null, c), b.contextName = i.contextName, v(b);
				j && y(g, function (a) {
					F(a, {}, {})
				});
		...
		function L() {
			...
			for (w(); H.length; ) {
				a = H.shift();
				if (null === a[0])
					return v(B("mismatch", "Mismatched anonymous define() module: " + a[a.length - 1]));
				E(a)
			}
		}
		var W, Z, i, N, X, k = {
							...
							if (!this.defining) {
								...
								if (1 > this.depCount && !this.defined) {
									if (I(n)) {
										...
										if (a)
											return a.requireMap = this.map, a.requireModules = [this.map.id], a.requireType = "define", v(this.error = a)
									} else
										e = n;
									this.exports = e;
									if (this.map.isDefine && !this.ignore && (q[c] = e, l.onResourceLoad))
										l.onResourceLoad(i, this.map, this.depMaps);
									x(c);
									this.defined = !0
								}
								this.defining = !1;
		...
		i = {
			...
			makeRequire : function (a, f) {
				function d(e, c, h) {
					...
					if ("string" === typeof e) {
						if (I(c))
							return v(B("requireargs", "Invalid require call"), h);
						if (a && s(N, e))
							return N[e](p[a.id]);
						if (l.get)
							return l.get(i, e, a, d);
						g = j(e, a, !1, !0);
						g = g.id;
			...
			completeLoad: function(a) {
				...
				if (!b && !s(q, a) && c && !c.inited) {
					if (k.enforceDefine && (!d || !ba(d)))
						return z(a) ? void 0 : v(B("nodefine", "No define call for " + a, null, [a]));
	...
	if ("undefined" === typeof define) {
		...
		if (A && (x = w.head = document.getElementsByTagName("head")[0], D = document.getElementsByTagName("base")[0]))
			x = w.head = D.parentNode;
		...
		A && M(document.getElementsByTagName("script"), function (b) {
			x || (x = b.parentNode);
			if (t = b.getAttribute("data-main"))
				return r.baseUrl || (E = t.split("/"), Q = E.pop(), fa = E.length ? E.join("/") + "/" : "./", r.baseUrl = fa, t = Q), t = t.replace(ea, ""), r.deps = r.deps ? r.deps.concat(t) : [t], !0
		});
