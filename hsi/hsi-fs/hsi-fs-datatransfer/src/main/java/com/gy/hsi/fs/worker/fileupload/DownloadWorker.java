	...
	public static List<String> doIt() {
		...
		while (true) {
			...
			for (TFsFileMetaData d : list) {
				actionDownloadTfsInThreadpool(d.getFileStorageId(), counterList,
						latch);
			}
...
//				latch.await(15, TimeUnit.MINUTES);
	...
	private static ResultCounter actionDdownloadFTPFiles() {
		...
		try {
			recurFtpFiles(sysConf.getFtpRootDir()+"/"+"company", resultCounter);
			recurFtpFiles(sysConf.getFtpRootDir()+"/"+"person", resultCounter);
		} catch (Exception e) {
	...
	private static List<TFsFileMetaData> filterValidRecords(List<TFsFileMetaData> list) {
		...
		for (TFsFileMetaData d : list) {
			fileId = d.getFileId();
			fileSizeInDB = d.getFileSizeBytes();

//			if ((0 < fileSizeInDB)
//					&& (fileSizeInDB == getFileSizeInLocalDir(fileId,
//							systemConfig))) {
			if (0 < fileSizeInDB) {
				continue;
			}
	...
	@SuppressWarnings("unused")
	private static long getFileSizeInLocalDir(String fileId, SystemConfig systemConfig) {
		String localFilePath = StringUtils.joinFilePath(
				systemConfig.getDownloadLocalpath(),
				FileIdHelper.formatFileId(fileId));

		try {
			return FileUtils.sizeOf(new File(localFilePath));
