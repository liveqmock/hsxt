...
import com.gy.hsxt.rabbitmq.common.rabbitmq.impl.HessionCodecFactory;

/**
 * 
 * @ClassName: EventControlConfig 
 * @Description: 事件控制配置,主要初始化rabbitmq配置信息
 * @author Lee 
 * @date 2015-7-3 下午2:46:38
 */
public class EventControlConfig {

	private final static int DEFAULT_PORT = Integer.parseInt(HsPropertiesConfigurer.getProperty(ConfigConstant.RABBITMQ_PORT));
	
	private final static String DEFAULT_USERNAME = HsPropertiesConfigurer.getProperty(ConfigConstant.RABBITMQ_USERNAME);
	
	private final static String DEFAULT_PASSWORD = HsPropertiesConfigurer.getProperty(ConfigConstant.RABBITMQ_PASSWD);
	...
	public EventControlConfig(String serverHost,int maxconsumers) {
		   this(serverHost,DEFAULT_PORT,DEFAULT_USERNAME,DEFAULT_PASSWORD,null,0,maxconsumers,DEFAULT_PROCESS_THREAD_NUM,new HessionCodecFactory());
	}
