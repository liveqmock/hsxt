...
public class RestfulMgrImpl implements RestfulMgr {
	...
	/**
	 * 由zhangysh修改, 解决在linux服务器上, 修改user.dir目录为非tomcat默认目录时无法下载文件问题
	 * 
	 * @param remoteUrl
	 *            远程地址
	 * @param fileName
	...
	public String downloadFromServer(RemoteUrl remoteUrl, String fileName,
			String localFileDir, boolean isTransfer2Classpath, int retryTimes,
			int retrySleepSeconds) throws Exception {

		String userDir = System.getProperty("user.dir");

		{
			userDir = this.formatPath(userDir, "./");

			localFileDir = userDir + this.formatPath(localFileDir, "./");
			localFileDir = this.replace2RealPath(localFileDir);
			
			LOGGER.warn("userDir=" + userDir);
			LOGGER.warn("localFileDir=" + localFileDir);
		}

		// 临时下载目录
		// String tmpFileDir = userDir + "disconf/download";
		String tmpFileDir = userDir;

		// 待下载的路径
		// String localFilePath = OsUtil.pathJoin(localFileDir, fileName);
		String localFilePath = this.replace2RealPath(localFileDir + fileName);
		String tmpFilePath = this.replace2RealPath(OsUtil.pathJoin(tmpFileDir, fileName));

		LOGGER.warn("localFilePath=" + localFilePath);
		LOGGER.warn("tmpFilePath=" + tmpFilePath);

		// 唯一标识化本地文件
		String tmpFilePathUnique = MyStringUtils.getRandomName(tmpFilePath);

		LOGGER.warn("tmpFilePathUnique=" + tmpFilePathUnique);
		...
		try {
			// 可重试的下载
			this.retry4ConfDownload(remoteUrl, tmpFilePathUniqueFile, retryTimes,
					retrySleepSeconds);
			...
			if (isTransfer2Classpath) {
				...
				if (classpathFile != null) {
					...
					OsUtil.transferFileAtom(tmpFilePathUniqueFile,
							classpathFile, true);
			...
			LOGGER.warn("download file failed, using previous download file.  localFile.getAbsolutePath()=" + localFile.getAbsolutePath(),
					e);
		}
		...
		if (!ConfigLoaderUtils.CLASS_PATH.isEmpty()) {
			String relativePathString = OsUtil.getRelativePath(localFile,
	 ...
	 * @return
	 */
	private String formatPath(String path, String defaultPath) {
	 ...
	 * @return
	 */
	private String replace2RealPath(String path) {
