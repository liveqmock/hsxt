...
import com.gy.hsi.ds.common.thirds.dsp.common.exception.RemoteException;
 ...
 */
public class ZookeeperDriverImpl implements IZooKeeperDriver, InitializingBean, DisposableBean {

    protected static final Logger LOG = LoggerFactory.getLogger(IZooKeeperDriver.class);
    ...
    public ZkDisconfData getDisconfData(List<App> appList,String env, String version, DisConfigTypeEnum disConfigTypeEnum,
        ...
        try
        {
            for(int i=0;i<appList.size();i++){
                app=appList.get(i);
                baseUrl= ZooPathMgr.getZooBaseUrl(zooConfig.getZookeeperUrlPrefix(), app.getAppName(), env, version);
                baseUrls.add(ZooPathMgr.getFileZooPath(baseUrl));
            }
            ZookeeperMgr zooKeeperMgr = ZookeeperMgr.getInstance();
            ZooKeeper zooKeeper = zooKeeperMgr.getZk();
            if (disConfigTypeEnum.equals(DisConfigTypeEnum.FILE))
            {
                ...
                return getDisconfData(baseUrls, keyName, zooKeeper);
     ...
     * @throws InterruptedException
     */
    private ZkDisconfData getDisconfData(List<String> paths, String keyName, ZooKeeper zooKeeper)
        ...
        String path = null;
        String curPath = null;
        List<String> secChiList = null;
        for (int i = 0; i < paths.size(); i++)
        {
            path = paths.get(i);
            curPath = path + "/" + keyName;
            if (zooKeeper.exists(curPath, false) != null)
            {
                secChiList = zooKeeper.getChildren(curPath, false);
                // list
                for (String secKey : secChiList)
                {
