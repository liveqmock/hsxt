 ...
 * 获取location中search的值 @return {string} value 返回configId
 */
Util.param.getBusId = function () {
    var ids = /busId=(\d+)&?/.exec(location.search);
    var id = ids ? ids[1] : '';
    return id;
};

/*
 * 获取location中path的值 @return {string} value 返回triggerName
 */
Util.param.getTriggerName = function () {
    var triggerNames = /triggerName=(\w+)&?/.exec(location.search);
    var triggerName = triggerNames ? triggerNames[1] : '';
    return triggerName;
};

/*
 * 获取location中path的值 @return {string} value 返回triggerGroup
 */
Util.param.getTriggerGroup = function () {
    var triggerGroups = /triggerGroup=(\w+)&?/.exec(location.search);
    var triggerGroup = triggerGroups ? triggerGroups[1] : '';
    return triggerGroup;
};
